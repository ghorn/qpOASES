...
#define HESSIANTYPE                                                            \
  HST_UNKNOWN /**< Hessian type, see documentation of QProblemB class          \
                 constructor. */
...
static void mdlStart(SimStruct *S) {
  ...
  if (mxIsEmpty(in_H) == 1) {
    if ((HESSIANTYPE != HST_ZERO) && (HESSIANTYPE != HST_IDENTITY)) {
      ...
      mexErrMsgTxt("ERROR (qpOASES): Hessian can only be empty if type is set "
                   "to HST_ZERO or HST_IDENTITY!");
#endif
      return;
    }
