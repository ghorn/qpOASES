...
int_t QProblem_qpOASES(int_t nV, int_t nC, HessianType hessianType, int_t nP,
                       SymmetricMatrix *H, double *g, Matrix *A, double *lb,
                       double *ub, double *lbA, double *ubA, int_t nWSRin,
                       real_t maxCpuTimeIn, const double *const x0,
                       Options *options, int_t nOutputs, mxArray *plhs[],
                       const double *const guessedBounds,
                       const double *const guessedConstraints,
                       const double *const _R, BooleanType isSparse) {
  int_t nWSRout;
  ...
  returnvalue = QP->init(H, g, A, lb, ub, lbA, ubA, nWSRout, &maxCpuTimeOut, x0,
                         0, (guessedBounds != 0) ? &bounds : 0,
  ...
  for (int_t k = 0; k < nP; ++k) {
    if (k > 0) {
      ...
      nWSRout = nWSRin;
      maxCpuTimeOut = (maxCpuTimeIn >= 0.0) ? maxCpuTimeIn : INFTY;
      returnvalue = QP->hotstart(g_current, lb_current, ub_current, lbA_current,
                                 ubA_current, nWSRout, &maxCpuTimeOut);
    }

    /* write results into output vectors */
    obtainOutputs(k, QP, returnvalue, nWSRout, maxCpuTimeOut, nOutputs, plhs,
                  nV, nC);
  }
...
int_t QProblemB_qpOASES(int_t nV, HessianType hessianType, int_t nP,
                        SymmetricMatrix *H, double *g, double *lb, double *ub,
  ...
  for (int_t k = 0; k < nP; ++k) {
    if (k > 0) {
      ...
      nWSRout = nWSRin;
      maxCpuTimeOut = (maxCpuTimeIn >= 0.0) ? maxCpuTimeIn : INFTY;
      returnvalue = QP.hotstart(g_current, lb_current, ub_current, nWSRout,
                                &maxCpuTimeOut);
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  int H_idx = -1, g_idx = -1, A_idx = -1, lb_idx = -1, ub_idx = -1,
      lbA_idx = -1, ubA_idx = -1;
  ...
  /* 1a) Ensure that qpOASES is called with a feasible number of input
   * arguments. */
  if ((nrhs < 4) || (nrhs > 9)) {
    myMexErrMsgTxt("ERROR (qpOASES): Invalid number of input arguments!\nType "
                   "'help qpOASES' for further information.");
    return;
  }
  ...
  /*     Choose between QProblem and QProblemB object and assign the
   * corresponding indices of the input pointer array in to order to access QP
   * data correctly. */
  g_idx = 1;
  ...
  nP = (int_t)mxGetN(
      prhs[g_idx]); /* number of columns of the gradient matrix (vectors series
                       have to be stored columnwise!) */
  ...
  if (isSimplyBoundedQp == BT_TRUE) {
    ...
    if ((!mxIsEmpty(prhs[4])) && (mxIsStruct(prhs[4]))) {
      myMexErrMsgTxt("ERROR (qpOASES): Fifth input argument must not be a "
                     "struct when solving QP with general constraints!\nType "
                     "'help qpOASES' for further information.");
      return;
    }
  ...
  /* 1) Simply bounded QP. */
  if ((isSimplyBoundedQp == BT_TRUE) || ((numberOfColumns == 1) && (nV != 1))) {
    lb_idx = 2;
    ub_idx = 3;
  ...
  if (nC > 0) {
    ...
    if (mxGetN(prhs[A_idx]) != nV) {
      char msg[MAX_STRING_LENGTH];
      snprintf(msg, MAX_STRING_LENGTH,
               "ERROR (qpOASES): Constraint matrix input dimension mismatch "
               "(%ld != %d)!",
  ...
  if (smartDimensionCheck(&g, nV, nP, BT_FALSE, prhs, g_idx) !=
      SUCCESSFUL_RETURN)
    return;

  if (smartDimensionCheck(&lb, nV, nP, BT_TRUE, prhs, lb_idx) !=
      SUCCESSFUL_RETURN)
    return;

  if (smartDimensionCheck(&ub, nV, nP, BT_TRUE, prhs, ub_idx) !=
      SUCCESSFUL_RETURN)
    return;
  ...
  if (nC > 0) {
    if (smartDimensionCheck(&lbA, nC, nP, BT_TRUE, prhs, lbA_idx) !=
        SUCCESSFUL_RETURN)
      return;

    if (smartDimensionCheck(&ubA, nC, nP, BT_TRUE, prhs, ubA_idx) !=
        SUCCESSFUL_RETURN)
      return;
  }
  ...
  if (nC == 0) {
    ...
    QProblemB_qpOASES(nV, hessianType, nP, H, g, lb, ub, nWSRin, maxCpuTimeIn,
                      x0, &options, nlhs, plhs, guessedBounds, R);
    ...
    if (A == 0) {
      myMexErrMsgTxt("ERROR (qpOASES): Internal interface error related to "
                     "constraint matrix!");
      return;
    }

    /* Call qpOASES (using QProblem class). */
    QProblem_qpOASES(nV, nC, hessianType, nP, H, g, A, lb, ub, lbA, ubA, nWSRin,
                     maxCpuTimeIn, x0, &options, nlhs, plhs, guessedBounds,
                     guessedConstraints, R, isSparse);
