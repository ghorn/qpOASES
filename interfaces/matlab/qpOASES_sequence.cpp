 ...
 */
int_t QProblemB_init(int_t handle, SymmetricMatrix *H, real_t *g, const real_t *const lb,
                     const real_t *const ub, int_t nWSRin, real_t maxCpuTimeIn,
                     const double *const x0, Options *options, int_t nOutputs, mxArray *plhs[],
                     const double *const guessedBounds, const double *const _R) {
  ...
  obtainOutputs(0, globalQPB, returnvalue, nWSRout, maxCpuTimeOut, nOutputs, plhs, nV, 0, handle);
 ...
 */
int_t SQProblem_init(int_t handle, SymmetricMatrix *H, real_t *g, Matrix *A, const real_t *const lb,
                     const real_t *const ub, const real_t *const lbA, const real_t *const ubA,
                     int_t nWSRin, real_t maxCpuTimeIn, const double *const x0, Options *options,
                     int_t nOutputs, mxArray *plhs[], const double *const guessedBounds,
                     const double *const guessedConstraints, const double *const _R) {
  int_t nWSRout = nWSRin;
  real_t maxCpuTimeOut = (maxCpuTimeIn >= 0.0) ? maxCpuTimeIn : INFTY;
  ...
  /* 3) Assign lhs arguments. */
  obtainOutputs(0, globalSQP, returnvalue, nWSRout, maxCpuTimeOut, nOutputs, plhs, nV, nC, handle);
 ...
 */
int_t QProblemB_hotstart(int_t handle, const real_t *const g, const real_t *const lb,
                         const real_t *const ub, int_t nWSRin, real_t maxCpuTimeIn,
  ...
  obtainOutputs(0, globalQPB, returnvalue, nWSRout, maxCpuTimeOut, nOutputs, plhs, nV, 0);
 ...
 */
int_t QProblem_hotstart(int_t handle, const real_t *const g, const real_t *const lb,
                        const real_t *const ub, const real_t *const lbA, const real_t *const ubA,
                        int_t nWSRin, real_t maxCpuTimeIn, Options *options, int_t nOutputs,
                        mxArray *plhs[]) {
  int_t nWSRout = nWSRin;
  real_t maxCpuTimeOut = (maxCpuTimeIn >= 0.0) ? maxCpuTimeIn : INFTY;
  ...
  /* 2) Assign lhs arguments. */
  obtainOutputs(0, globalSQP, returnvalue, nWSRout, maxCpuTimeOut, nOutputs, plhs, nV, nC);
...
int_t SQProblem_hotstart(int_t handle, SymmetricMatrix *H, real_t *g, Matrix *A,
                         const real_t *const lb, const real_t *const ub, const real_t *const lbA,
                         const real_t *const ubA, int_t nWSRin, real_t maxCpuTimeIn,
                         Options *options, int_t nOutputs, mxArray *plhs[]) {
  int_t nWSRout = nWSRin;
  real_t maxCpuTimeOut = (maxCpuTimeIn >= 0.0) ? maxCpuTimeIn : INFTY;
  ...
  switch (returnvalue) {
    ...
    case RET_QP_INFEASIBLE: break;

    default: myMexErrMsgTxt("ERROR (qpOASES): Hotstart failed."); return -1;
  }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if ((strcmp(typeString, "i") == 0) || (strcmp(typeString, "I") == 0)) {
    ...
    if (((H_idx >= 0) && (mxIsDouble(prhs[H_idx]) == 0)) || (mxIsDouble(prhs[g_idx]) == 0)) {
    ...
    if (auxInput_idx >= 0)
      setupAuxiliaryInputs(prhs[auxInput_idx], nV, nC, &hessianType, &x0, &guessedBounds,
                           &guessedConstraints, &R_for);
    ...
    if (H_idx >= 0)
      setupHessianMatrix(prhs[H_idx], nV, &(globalQP->H), &(globalQP->Hir), &(globalQP->Hjc),
                         &(globalQP->Hv));

    /* make a deep-copy of the user-specified constraint matrix (possibly sparse) */
    if ((nC > 0) && (A_idx >= 0))
      setupConstraintMatrix(prhs[A_idx], nV, nC, &(globalQP->A), &(globalQP->Air), &(globalQP->Ajc),
                            &(globalQP->Av));
    ...
    /* Call qpOASES. */
    if (isSimplyBoundedQp == BT_TRUE) {
      QProblemB_init(handle, globalQP->H, g, lb, ub, nWSRin, maxCpuTimeIn, x0, &options, nlhs, plhs,
                     guessedBounds, R);
    } else {
      SQProblem_init(handle, globalQP->H, g, globalQP->A, lb, ub, lbA, ubA, nWSRin, maxCpuTimeIn,
                     x0, &options, nlhs, plhs, guessedBounds, guessedConstraints, R);
    }
  ...
  if ((strcmp(typeString, "h") == 0) || (strcmp(typeString, "H") == 0)) {
    ...
    /* call qpOASES */
    if (isSimplyBoundedQp == BT_TRUE) {
      QProblemB_hotstart(handle, g, lb, ub, nWSRin, maxCpuTimeIn, &options, nlhs, plhs);
    } else {
      QProblem_hotstart(handle, g, lb, ub, lbA, ubA, nWSRin, maxCpuTimeIn, &options, nlhs, plhs);
    }
  ...
  if ((strcmp(typeString, "m") == 0) || (strcmp(typeString, "M") == 0)) {
    ...
    if (nV != (uint_t)globalQP->getNV() || nC != (uint_t)globalQP->getNC()) {
      myMexErrMsgTxt(
          "ERROR (qpOASES): QP dimensions must be constant during a sequence! Try creating a new "
          "QP instance instead.");
      return;
    }
    ...
    /* Call qpOASES */
    SQProblem_hotstart(handle, globalQP->H, g, globalQP->A, lb, ub, lbA, ubA, nWSRin, maxCpuTimeIn,
                       &options, nlhs, plhs);
  ...
  if ((strcmp(typeString, "e") == 0) || (strcmp(typeString, "E") == 0)) {
    ...
    if (returnvalue != SUCCESSFUL_RETURN) {
      char msg[MAX_STRING_LENGTH];
      snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Couldn't solve current EQP (code %d)!",
               returnvalue);
      myMexErrMsgTxt(msg);
      return;
    }
