 ...
 *Alexander Buchner \version 3.2 \date 2007-2017
 *
 ...
 */
int_t QProblemB_init(int_t handle, SymmetricMatrix *H, real_t *g,
                     const real_t *const lb, const real_t *const ub,
                     int_t nWSRin, real_t maxCpuTimeIn, const double *const x0,
                     Options *options, int_t nOutputs, mxArray *plhs[],
  ...
  returnvalue = globalQPB->init(H, g, lb, ub, nWSRout, &maxCpuTimeOut, x0, 0,
                                (guessedBounds != 0) ? &bounds : 0, _R);

  /* 3) Assign lhs arguments. */
  obtainOutputs(0, globalQPB, returnvalue, nWSRout, maxCpuTimeOut, nOutputs,
                plhs, nV, 0, handle);
 ...
 */
int_t SQProblem_init(int_t handle, SymmetricMatrix *H, real_t *g, Matrix *A,
                     const real_t *const lb, const real_t *const ub,
    ...
    for (int_t i = 0; i < nC; i++) {
      if (isEqual(guessedConstraints[i], -1.0) == BT_TRUE) {
        ...
        snprintf(msg, MAX_STRING_LENGTH,
                 "ERROR (qpOASES): Only {-1, 0, 1} allowed for status of "
                 "constraints!");
        myMexErrMsgTxt(msg);
        return -1;
      }
  ...
  returnvalue =
      globalSQP->init(H, g, A, lb, ub, lbA, ubA, nWSRout, &maxCpuTimeOut, x0, 0,
                      (guessedBounds != 0) ? &bounds : 0,
                      (guessedConstraints != 0) ? &constraints : 0, _R);

  /* 3) Assign lhs arguments. */
  obtainOutputs(0, globalSQP, returnvalue, nWSRout, maxCpuTimeOut, nOutputs,
                plhs, nV, nC, handle);
 ...
 */
int_t QProblemB_hotstart(int_t handle, const real_t *const g,
                         const real_t *const lb, const real_t *const ub,
                         int_t nWSRin, real_t maxCpuTimeIn, Options *options,
  ...
  globalQPB->setOptions(*options);
  returnValue returnvalue =
      globalQPB->hotstart(g, lb, ub, nWSRout, &maxCpuTimeOut);
 ...
 */
int_t QProblem_hotstart(int_t handle, const real_t *const g,
                        const real_t *const lb, const real_t *const ub,
                        const real_t *const lbA, const real_t *const ubA,
                        int_t nWSRin, real_t maxCpuTimeIn, Options *options,
                        int_t nOutputs, mxArray *plhs[]) {
  ...
  globalSQP->setOptions(*options);
  returnValue returnvalue =
      globalSQP->hotstart(g, lb, ub, lbA, ubA, nWSRout, &maxCpuTimeOut);
 ...
 */
int_t SQProblem_hotstart(int_t handle, SymmetricMatrix *H, real_t *g, Matrix *A,
                         const real_t *const lb, const real_t *const ub,
                         const real_t *const lbA, const real_t *const ubA,
                         int_t nWSRin, real_t maxCpuTimeIn, Options *options,
                         int_t nOutputs, mxArray *plhs[]) {
  ...
  globalSQP->setOptions(*options);
  returnValue returnvalue =
      globalSQP->hotstart(H, g, A, lb, ub, lbA, ubA, nWSRout, &maxCpuTimeOut);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  int_t H_idx = -1, g_idx = -1, A_idx = -1, lb_idx = -1, ub_idx = -1,
        lbA_idx = -1, ubA_idx = -1;
  int_t x0_idx = -1, auxInput_idx = -1;

  BooleanType isSimplyBoundedQp = BT_FALSE;
#ifdef SOLVER_MA57
  BooleanType isSparse = BT_TRUE; /* This will be set to BT_FALSE later if a
                                     dense matrix is encountered. */
#else
  ...
  if ((nrhs < 5) || (nrhs > 10)) {
    if (nrhs != 2) {
      myMexErrMsgTxt(
          ...
          "qpOASES_sequence' for further information.");
      return;
    }
  }
  ...
  if ((strcmp(typeString, "i") != 0) && (strcmp(typeString, "I") != 0) &&
    ...
    myMexErrMsgTxt("ERROR (qpOASES): Undefined first input argument!\nType "
                   "'help qpOASES_sequence' for further information.");
    return;
  }
  ...
  if ((strcmp(typeString, "i") == 0) || (strcmp(typeString, "I") == 0)) {
    ...
    if ((H_idx >= 0) &&
        ((mxGetN(prhs[H_idx]) != nV) || (mxGetM(prhs[H_idx]) != nV))) {
    ...
    if (isSimplyBoundedQp == BT_TRUE) {
      ...
      if (smartDimensionCheck(&g, nV, 1, BT_FALSE, prhs, 2) !=
          SUCCESSFUL_RETURN)
        return;

      if (smartDimensionCheck(&lb, nV, 1, BT_TRUE, prhs, 3) !=
          SUCCESSFUL_RETURN)
        return;

      if (smartDimensionCheck(&ub, nV, 1, BT_TRUE, prhs, 4) !=
          SUCCESSFUL_RETURN)
        return;
      ...
      if (mxIsDouble(prhs[A_idx]) == 0) {
        myMexErrMsgTxt("ERROR (qpOASES): All data has to be provided in double "
                       "precision!");
        return;
      }
      ...
      if (smartDimensionCheck(&g, nV, 1, BT_FALSE, prhs, g_idx) !=
          SUCCESSFUL_RETURN)
        return;

      if (smartDimensionCheck(&lb, nV, 1, BT_TRUE, prhs, lb_idx) !=
          SUCCESSFUL_RETURN)
        return;

      if (smartDimensionCheck(&ub, nV, 1, BT_TRUE, prhs, ub_idx) !=
          SUCCESSFUL_RETURN)
        return;

      if (smartDimensionCheck(&lbA, nC, 1, BT_TRUE, prhs, lbA_idx) !=
          SUCCESSFUL_RETURN)
        return;

      if (smartDimensionCheck(&ubA, nC, 1, BT_TRUE, prhs, ubA_idx) !=
          SUCCESSFUL_RETURN)
        return;
    ...
    if (smartDimensionCheck(&x0, nV, 1, BT_TRUE, prhs, x0_idx) !=
        SUCCESSFUL_RETURN)
      return;

    if (auxInput_idx >= 0)
      setupAuxiliaryInputs(prhs[auxInput_idx], nV, nC, &hessianType, &x0,
                           &guessedBounds, &guessedConstraints, &R_for);
    ...
    /* allocate instance */
    handle = allocateQPInstance(nV, nC, hessianType, isSimplyBoundedQp,
                                isSparse, &options);
     ...
     */
    if (H_idx >= 0)
      setupHessianMatrix(prhs[H_idx], nV, &(globalQP->H), &(globalQP->Hir),
                         &(globalQP->Hjc), &(globalQP->Hv));

    /* make a deep-copy of the user-specified constraint matrix (possibly
     * sparse) */
    if ((nC > 0) && (A_idx >= 0))
      setupConstraintMatrix(prhs[A_idx], nV, nC, &(globalQP->A),
                            &(globalQP->Air), &(globalQP->Ajc),
                            &(globalQP->Av));
    ...
    /* Call qpOASES. */
    if (isSimplyBoundedQp == BT_TRUE) {
      QProblemB_init(handle, globalQP->H, g, lb, ub, nWSRin, maxCpuTimeIn, x0,
                     &options, nlhs, plhs, guessedBounds, R);
    } else {
      SQProblem_init(handle, globalQP->H, g, globalQP->A, lb, ub, lbA, ubA,
                     nWSRin, maxCpuTimeIn, x0, &options, nlhs, plhs,
  ...
  if ((strcmp(typeString, "h") == 0) || (strcmp(typeString, "H") == 0)) {
    ...
    if ((mxIsDouble(prhs[1]) == false) || (mxIsScalar(prhs[1]) == false)) {
      myMexErrMsgTxt(
          "ERROR (qpOASES): Expecting a handle to QP object as second "
    ...
    /* call qpOASES */
    if (isSimplyBoundedQp == BT_TRUE) {
      QProblemB_hotstart(handle, g, lb, ub, nWSRin, maxCpuTimeIn, &options,
                         nlhs, plhs);
    } else {
      QProblem_hotstart(handle, g, lb, ub, lbA, ubA, nWSRin, maxCpuTimeIn,
                        &options, nlhs, plhs);
    }
  ...
  if ((strcmp(typeString, "m") == 0) || (strcmp(typeString, "M") == 0)) {
    ...
    if (mxIsEmpty(prhs[2]) == 1) {
      H_idx = -1;
      nV = (uint_t)mxGetM(
          prhs[g_idx]); /* if Hessian is empty, row number of gradient vector */
    } else {
      H_idx = 2;
      nV = (uint_t)mxGetM(prhs[H_idx]); /* row number of Hessian matrix */
    }
    ...
    /* ensure that data is given in double precision */
    if (((H_idx >= 0) && (mxIsDouble(prhs[H_idx]) == 0)) ||
        (mxIsDouble(prhs[g_idx]) == 0) || (mxIsDouble(prhs[A_idx]) == 0)) {
      myMexErrMsgTxt(
    ...
    if (nV != (uint_t)globalQP->getNV() || nC != (uint_t)globalQP->getNC()) {
      myMexErrMsgTxt("ERROR (qpOASES): QP dimensions must be constant during a "
                     "sequence! Try creating a new QP instance instead.");
      return;
    }
    ...
    /* Call qpOASES */
    SQProblem_hotstart(handle, globalQP->H, g, globalQP->A, lb, ub, lbA, ubA,
                       nWSRin, maxCpuTimeIn, &options, nlhs, plhs);
  ...
  if ((strcmp(typeString, "e") == 0) || (strcmp(typeString, "E") == 0)) {
    ...
    if (smartDimensionCheck(&g, nV, nRHS, BT_FALSE, prhs, g_idx) !=
        SUCCESSFUL_RETURN)
      return;

    if (smartDimensionCheck(&lb, nV, nRHS, BT_TRUE, prhs, lb_idx) !=
        SUCCESSFUL_RETURN)
      return;

    if (smartDimensionCheck(&ub, nV, nRHS, BT_TRUE, prhs, ub_idx) !=
        SUCCESSFUL_RETURN)
      return;

    if (smartDimensionCheck(&lbA, nC, nRHS, BT_TRUE, prhs, lbA_idx) !=
        SUCCESSFUL_RETURN)
      return;

    if (smartDimensionCheck(&ubA, nC, nRHS, BT_TRUE, prhs, ubA_idx) !=
        SUCCESSFUL_RETURN)
      return;
    ...
    returnValue returnvalue =
        globalQP->sqp->solveCurrentEQP(nRHS, g, lb, ub, lbA, ubA, x_out, y_out);
    ...
    if (returnvalue != SUCCESSFUL_RETURN) {
      char msg[MAX_STRING_LENGTH];
      snprintf(msg, MAX_STRING_LENGTH,
               "ERROR (qpOASES): Couldn't solve current EQP (code %d)!",
               returnvalue);
      myMexErrMsgTxt(msg);
      return;
    }
