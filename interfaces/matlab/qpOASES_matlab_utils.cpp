...
void deleteQPInstance(int_t handle) {
  ...
  if (instance != 0) {
    for (std::vector<QPInstance *>::iterator itor = g_instances.begin(); itor != g_instances.end();
         ++itor)
  ...
  if (mxIsEmpty(prhs[idx])) {
    ...
    if ((emptyAllowed == BT_TRUE) || (idx == 0)) /* idx==0 used for auxInput */
    {
      *input = 0;
      ...
      if (idx > 0)
        snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Empty argument %d not allowed!",
                 idx + 1);
      myMexErrMsgTxt(msg);
        ...
        *input = (real_t *)mxGetPr(prhs[idx]);
        ...
        if (idx > 0)
          ...
          snprintf(msg, MAX_STRING_LENGTH,
                   "ERROR (qpOASES): Input dimension mismatch for some auxInput entry ([%ld,%ld] "
                   "~= [%d,%d]).",
...
BooleanType containsNaNorInf(const mxArray *prhs[], int_t rhs_index, bool mayContainInf) {
  ...
  if (containsNaN((real_t *)mxGetPr(prhs[rhs_index]), dim) == BT_TRUE) {
    snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Argument %d contains 'NaN' !",
             rhs_index + 1);
    myMexErrMsgTxt(msg);
    return BT_TRUE;
  }
  ...
  if (mayContainInf == 0) {
    if (containsInf((real_t *)mxGetPr(prhs[rhs_index]), dim) == BT_TRUE) {
      snprintf(msg, MAX_STRING_LENGTH, "ERROR (qpOASES): Argument %d contains 'Inf' !",
               rhs_index + 1);
      myMexErrMsgTxt(msg);
      return BT_TRUE;
    }
 ...
 */
BooleanType hasOptionsValue(const mxArray *optionsPtr, const char *const optionString,
                            double **optionValue) {
  ...
  if (optionName == 0) {
    char msg[MAX_STRING_LENGTH];
    snprintf(msg, MAX_STRING_LENGTH,
             "Option struct does not contain entry '%s', using default value instead!",
             optionString);
    mexWarnMsgTxt(msg);
    return BT_FALSE;
  }
  ...
  if ((mxIsEmpty(optionName) == false) && (mxIsScalar(optionName) == true)) {
    *optionValue = mxGetPr(optionName);
    ...
    char msg[MAX_STRING_LENGTH];
    snprintf(msg, MAX_STRING_LENGTH, "Option '%s' is not a scalar, using default value instead!",
             optionString);
    mexWarnMsgTxt(msg);
    return BT_FALSE;
  }
 ...
 */
returnValue setupOptions(Options *options, const mxArray *optionsPtr, int_t &nWSRin,
                         real_t &maxCpuTime) {
  double *optionValue;
  ...
  if (mxGetNumberOfFields(optionsPtr) != 31)
    mexWarnMsgTxt(
        ...
        "'help qpOASES_options' for further information.");
...
returnValue setupAuxiliaryInputs(const mxArray *auxInput, uint_t nV, uint_t nC,
                                 HessianType *hessianType, double **x0, double **guessedBounds,
  ...
  if (curField == NULL)
    mexWarnMsgTxt(
        ...
        "qpOASES_auxInput' for further information.");
  else {
  ...
  if (curField == NULL)
    mexWarnMsgTxt(
        "auxInput struct does not contain entry 'x0'!\n         Type 'help qpOASES_auxInput' for "
        "further information.");
  else {
    *x0 = mxGetPr(curField);
    if (smartDimensionCheck(x0, nV, 1, BT_TRUE, ((const mxArray **)&curField), 0) !=
        SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
  ...
  if (curField == NULL)
    ...
    *guessedBounds = mxGetPr(curField);
    if (smartDimensionCheck(guessedBounds, nV, 1, BT_TRUE, ((const mxArray **)&curField), 0) !=
        SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
  ...
  if (curField == NULL)
    ...
    *guessedConstraints = mxGetPr(curField);
    if (smartDimensionCheck(guessedConstraints, nC, 1, BT_TRUE, ((const mxArray **)&curField), 0) !=
        SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
  ...
  if (curField == NULL)
    ...
    *R = mxGetPr(curField);
    if (smartDimensionCheck(R, nV, nV, BT_TRUE, ((const mxArray **)&curField), 0) !=
        SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
 ...
 */
returnValue allocateOutputs(int nlhs, mxArray *plhs[], int_t nV, int_t nC = 0, int_t nP = 1,
                            int_t handle = -1) {
 ...
 */
returnValue obtainOutputs(int_t k, QProblemB *qp, returnValue returnvalue, int_t _nWSRout,
                          double _cpuTime, int nlhs, mxArray *plhs[], int_t nV, int_t nC = 0,
                          int_t handle = -1) {
  /* Create output vectors and assign pointers to them. */
  int_t curIdx = 0;
 ...
 */
returnValue setupHessianMatrix(const mxArray *prhsH, int_t nV, SymmetricMatrix **H,
                               sparse_int_t **Hir, sparse_int_t **Hjc, real_t **Hv) {
  if (prhsH == 0)
    return SUCCESSFUL_RETURN;
 ...
 */
returnValue setupConstraintMatrix(const mxArray *prhsA, int_t nV, int_t nC, Matrix **A,
                                  sparse_int_t **Air, sparse_int_t **Ajc, real_t **Av) {
  if (prhsA == 0)
    return SUCCESSFUL_RETURN;
