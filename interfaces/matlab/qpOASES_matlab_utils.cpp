...
int_t allocateQPInstance(int_t nV, int_t nC, HessianType hessianType,
                         BooleanType isSimplyBounded, BooleanType isSparse,
                         const Options *options) {
  QPInstance *inst =
      new QPInstance(nV, nC, hessianType, isSimplyBounded, isSparse);
 ...
 */
returnValue smartDimensionCheck(real_t **input, uint_t m, uint_t n,
                                BooleanType emptyAllowed, const mxArray *prhs[],
                                int_t idx) {
    ...
    if (mxIsSparse(prhs[idx]) == 0) {
      if ((mxGetM(prhs[idx]) == m) && (mxGetN(prhs[idx]) == n)) {
        *input = (real_t *)mxGetPr(prhs[idx]);
        ...
        if (idx > 0)
          snprintf(msg, MAX_STRING_LENGTH,
                   "ERROR (qpOASES): Input dimension mismatch for argument %d "
                   "([%ld,%ld] ~= [%d,%d]).",
                   idx + 1, (long int)mxGetM(prhs[idx]),
                   (long int)mxGetN(prhs[idx]), (int)m, (int)n);
        else /* idx==0 used for auxInput */
          snprintf(msg, MAX_STRING_LENGTH,
                   "ERROR (qpOASES): Input dimension mismatch for some "
                   "auxInput entry ([%ld,%ld] ~= [%d,%d]).",
                   (long int)mxGetM(prhs[idx]), (long int)mxGetN(prhs[idx]),
                   (int)m, (int)n);
        myMexErrMsgTxt(msg);
        return RET_INVALID_ARGUMENTS;
      }
      ...
      if (idx > 0)
        snprintf(
            ...
            "ERROR (qpOASES): Vector argument %d must not be in sparse format!",
            idx + 1);
      else /* idx==0 used for auxInput */
        snprintf(
            msg, MAX_STRING_LENGTH,
...
BooleanType containsNaNorInf(const mxArray *prhs[], int_t rhs_index,
  ...
  if (mayContainInf == 0) {
    if (containsInf((real_t *)mxGetPr(prhs[rhs_index]), dim) == BT_TRUE) {
      snprintf(msg, MAX_STRING_LENGTH,
               "ERROR (qpOASES): Argument %d contains 'Inf' !", rhs_index + 1);
      myMexErrMsgTxt(msg);
      return BT_TRUE;
    }
  }
 ...
 */
returnValue convertFortranToC(const real_t *const M_for, int_t nV, int_t nC,
                              real_t *const M) {
  int_t i, j;
...
BooleanType hasOptionsValue(const mxArray *optionsPtr,
  ...
  if (optionName == 0) {
    char msg[MAX_STRING_LENGTH];
    snprintf(msg, MAX_STRING_LENGTH,
             "Option struct does not contain entry '%s', using default value "
             "instead!",
             optionString);
    mexWarnMsgTxt(msg);
    return BT_FALSE;
  ...
  if ((mxIsEmpty(optionName) == false) && (mxIsScalar(optionName) == true)) {
    *optionValue = mxGetPr(optionName);
    ...
    char msg[MAX_STRING_LENGTH];
    snprintf(msg, MAX_STRING_LENGTH,
             "Option '%s' is not a scalar, using default value instead!",
             optionString);
    mexWarnMsgTxt(msg);
    return BT_FALSE;
  }
 ...
 */
returnValue setupOptions(Options *options, const mxArray *optionsPtr,
                         int_t &nWSRin, real_t &maxCpuTime) {
  double *optionValue;
  ...
  if (mxGetNumberOfFields(optionsPtr) != 31)
    mexWarnMsgTxt("Options might be set incorrectly as struct has wrong number "
                  "of entries!\n         Type 'help qpOASES_options' for "
                  "further information.");
  ...
  if (hasOptionsValue(optionsPtr, "enableRamping", &optionValue) == BT_TRUE) {
    ...
    options->enableRamping =
        (REFER_NAMESPACE_QPOASES BooleanType)optionValueInt;
  }
  ...
  if (hasOptionsValue(optionsPtr, "enableCholeskyRefactorisation",
                      &optionValue) == BT_TRUE)
  ...
  if (hasOptionsValue(optionsPtr, "initialStatusBounds", &optionValue) ==
    ...
    options->initialStatusBounds =
        (REFER_NAMESPACE_QPOASES SubjectToStatus)optionValueInt;
  }
...
returnValue setupAuxiliaryInputs(const mxArray *auxInput, uint_t nV, uint_t nC,
                                 HessianType *hessianType, double **x0,
                                 double **guessedBounds,
                                 double **guessedConstraints, double **R) {
  mxArray *curField = 0;
  ...
  if (curField == NULL)
    mexWarnMsgTxt("auxInput struct does not contain entry 'x0'!\n         Type "
                  "'help qpOASES_auxInput' for further information.");
  else {
    *x0 = mxGetPr(curField);
    if (smartDimensionCheck(x0, nV, 1, BT_TRUE, ((const mxArray **)&curField),
                            0) != SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
  ...
  if (curField == NULL)
    ...
    *guessedBounds = mxGetPr(curField);
    if (smartDimensionCheck(guessedBounds, nV, 1, BT_TRUE,
                            ((const mxArray **)&curField),
                            0) != SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
  ...
  if (curField == NULL)
    ...
    *guessedConstraints = mxGetPr(curField);
    if (smartDimensionCheck(guessedConstraints, nC, 1, BT_TRUE,
                            ((const mxArray **)&curField),
                            0) != SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
  ...
  if (curField == NULL)
    ...
    *R = mxGetPr(curField);
    if (smartDimensionCheck(R, nV, nV, BT_TRUE, ((const mxArray **)&curField),
                            0) != SUCCESSFUL_RETURN)
      return RET_INVALID_ARGUMENTS;
  }
 ...
 */
returnValue allocateOutputs(int nlhs, mxArray *plhs[], int_t nV, int_t nC = 0,
                            int_t nP = 1, int_t handle = -1) {
      ...
      if (nlhs > curIdx) {
        ...
        if (nlhs > curIdx) {
          ...
          if (nlhs > curIdx) {
            ...
            if (curFieldNum >= 0)
              mxSetFieldByNumber(auxOutput, 0, curFieldNum,
                                 mxCreateDoubleMatrix(nV, nP, mxREAL));

            curFieldNum = mxAddField(auxOutput, "workingSetC");
            if (curFieldNum >= 0)
              mxSetFieldByNumber(auxOutput, 0, curFieldNum,
                                 mxCreateDoubleMatrix(nC, nP, mxREAL));

            curFieldNum = mxAddField(auxOutput, "cpuTime");
            if (curFieldNum >= 0)
              mxSetFieldByNumber(auxOutput, 0, curFieldNum,
                                 mxCreateDoubleMatrix(1, nP, mxREAL));
 ...
 */
returnValue obtainOutputs(int_t k, QProblemB *qp, returnValue returnvalue,
                          int_t _nWSRout, double _cpuTime, int nlhs,
                          mxArray *plhs[], int_t nV, int_t nC = 0,
                          int_t handle = -1) {
  /* Create output vectors and assign pointers to them. */
  int_t curIdx = 0;
        ...
        if (nlhs > curIdx) {
          ...
          if (nlhs > curIdx) {
            ...
            if (nC > 0) {
              ...
              /* cast successful? */
              if (problemPointer != NULL) {
                problemPointer->getWorkingSetConstraints(
                    &(workingSetC[k * nC]));
              } else {
...
returnValue setupHessianMatrix(const mxArray *prhsH, int_t nV,
                               SymmetricMatrix **H, sparse_int_t **Hir,
                               sparse_int_t **Hjc, real_t **Hv) {
  if (prhsH == 0)
    return SUCCESSFUL_RETURN;
...
returnValue setupConstraintMatrix(const mxArray *prhsA, int_t nV, int_t nC,
                                  Matrix **A, sparse_int_t **Air,
                                  sparse_int_t **Ajc, real_t **Av) {
  if (prhsA == 0)
    return SUCCESSFUL_RETURN;
