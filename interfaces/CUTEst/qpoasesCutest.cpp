...
int convertTripletToHbf(int m, int n, int nnz, double *vals, int *iRow, int *jCol, double *vals_hbf,
                        int *iRow_hbf, int *jCol_hbf) {
  int i, ii, j, k;
  ...
  /* Put row indices and values at the right places, use jCol_hbf[j] to track elements of the jth
   * column */
  for (k = 0; k < nnz; k++) {
...
int main() {
  ...
  CUTEST_csetup(&status, &funit, &iout, &io_buffer, &nVar, &nCon, xInit, lb, ub, yInit, lbA, ubA,
                isEq, isLinear, &e_order, &l_order, &v_order);
  if (status) {
  ...
  if (nCon > 0) {
    /* Evaluate sparse constraints at x=0 to get the rhs of equality constraints (they are not set
     * by csetup!) */
...
#if SOLVE_DENSE
  if (nCon > 0) {
    /* Evaluate dense constraints at x=0 to get the rhs of equality constraints (they are not set by
     * csetup!) */
    logical trans = 1;
