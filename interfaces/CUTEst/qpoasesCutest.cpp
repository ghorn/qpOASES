...
int convertTripletToHbf(int m, int n, int nnz, double *vals, int *iRow,
                        int *jCol, double *vals_hbf, int *iRow_hbf,
                        int *jCol_hbf) {
  int i, ii, j, k;
  ...
  /* Put row indices and values at the right places, use jCol_hbf[j] to track
   * elements of the jth column */
  for (k = 0; k < nnz; k++) {
...
int main() {
  ...
  /* Additional problem description: these variables are set and may also be
   * used by qpOASES */
  ...
  CUTEST_csetup(&status, &funit, &iout, &io_buffer, &nVar, &nCon, xInit, lb, ub,
                yInit, lbA, ubA, isEq, isLinear, &e_order, &l_order, &v_order);
  if (status) {
  ...
  if (nCon > 0) {
    /* Evaluate sparse constraints at x=0 to get the rhs of equality constraints
     * (they are not set by csetup!) */
    CUTEST_ccfsg(&status, &nVar, &nCon, xZero, cVal, &nnzj, &lj, J_val, J_var,
                 J_fun, &grad);
    if (status) {
    ...
    convertTripletToHbf(nCon, nVar, nnzj, J_val, J_fun, J_var, jacVal,
                        jacIndRow, jacIndCol);
  }
  delete[] J_val;
  delete[] J_fun;
  ...
  CUTEST_csh(&status, &nVar, &nCon, xInit, yInit, &nnzh, &lh, H_val, H_row,
             H_col);
  if (status) {
    printf("** CUTEst_csh: error, status = %d, aborting\n", status);
    exit(status);
  }
  /* Only upper triangular matrix is returned by CUTEst, to be safe, set H to be
   * the full matrix */
  int count = 0;
  ...
  convertTripletToHbf(nVar, nVar, nnzh, H_val, H_row, H_col, hesVal, hesIndRow,
                      hesIndCol);
...
#if SOLVE_DENSE
  if (nCon > 0) {
    /* Evaluate dense constraints at x=0 to get the rhs of equality constraints
     * (they are not set by csetup!) */
    logical trans = 1;
    jacDense = new double[nCon * nVar];
    CUTEST_ccfg(&status, &nVar, &nCon, xZero, cVal, &trans, &nVar, &nCon,
                jacDense, &grad);
  }
  ...
  for (int k = 0; k < nCon; k++)
    yOpt[k + nVar] = yInit[k]; // what about lambda for bound constraints?
  bOpt = Bounds(bInit);
  cOpt = Constraints(cInit);
  ret = qpDense.init(H_d, g, A_d, lb, ub, lbA, ubA, nWSR, &time, xOpt, yOpt,
                     &bOpt, &cOpt);
  ...
  bOpt = Bounds(bInit);
  cOpt = Constraints(cInit);
  // ret = qpSchur.init( H_s, g, A_s, lb, ub, lbA, ubA, nWSR, &time, xOpt, yOpt,
  // &bOpt, &cOpt );
  ...
  bOpt = Bounds(bInit);
  cOpt = Constraints(cInit);
  ret = qpSparse.init(H_s, g, A_s, lb, ub, lbA, ubA, nWSR, &time, xOpt, yOpt,
                      &bOpt, &cOpt);
