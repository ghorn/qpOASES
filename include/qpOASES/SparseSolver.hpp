...
class Ma27SparseSolver : public SparseSolver {
 ...
 private:
  ...
  double *a_ma27; /**< matrix/factor for MA27 (A in MA27).  If have_factorization is false, it
                     contains the matrix entries (and has length numNonzeros), otherwise the factor
                     (and has length la_ma27). */
  ...
  bool have_factorization; /**< flag indicating whether factorization for current matrix has already
                              been computed */
...
/**
 *	\brief Implementation of a dummy sparse solver. An error is thrown if a factorization is
 *attempted.
 *
 *	\author Dennis Janka
 *	\version 3.2
