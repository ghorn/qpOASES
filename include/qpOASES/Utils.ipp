 ...
 *   i s E q u a l
 */
inline BooleanType isEqual(real_t x, real_t y, real_t TOL) {
  if (getAbs(x - y) <= TOL)
    return BT_TRUE;
  else
    return BT_FALSE;
 ...
 *   i s Z e r o
 */
inline BooleanType isZero(real_t x, real_t TOL) {
  if (getAbs(x) <= TOL)
    return BT_TRUE;
  else
    return BT_FALSE;
 ...
 *   g e t M a x
 */
inline int_t getMax(int_t x, int_t y) { return (y < x) ? x : y; }

/*
 *   g e t M i n
 */
inline int_t getMin(int_t x, int_t y) { return (y > x) ? x : y; }
