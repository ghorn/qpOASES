 ...
 *	\author Andreas Waechter and Dennis Janka, based on QProblem.hpp by Hans Joachim Ferreau,
...
// void dgeqrf_(	const unsigned long *M, const unsigned long *N, double *A, const unsigned long
// *LDA, double *TAU, double *WORK, const unsigned long *LWORK, int *INFO );
/** Compute a QR factorization of a real M-by-N matrix A in single precision */
// void sgeqrf_(	const unsigned long *M, const unsigned long *N, float *A, const unsigned long
// *LDA, float *TAU, float *WORK, const unsigned long *LWORK, int *INFO );

/** Multiply C with orthogonal matrix Q**T as returned by geqrf (double precision) */
// void dormqr_(	const char *SIDE, const char *TRANS, const unsigned long *M, const unsigned
// long *N, const unsigned long *K, double *A, const unsigned long *LDA, double *TAU, double *C,
// const unsigned long *LDC, double *WORK, const unsigned long *LWORK, int *INFO );
/** Multiply C with orthogonal matrix Q**T as returned by geqrf (single precision) */
// void sormqr_(	const char *SIDE, const char *TRANS, const unsigned long *M, const unsigned
// long *N, const unsigned long *K, float *A, const unsigned long *LDA, float *TAU, float *C, const
// unsigned long *LDC, float *WORK, const unsigned long *LWORK, int *INFO );

/** Solve a triangular system (double precision) */
void dtrtrs_(const char *UPLO, const char *TRANS, const char *DIAG, const unsigned long *N,
             const unsigned long *NRHS, double *A, const unsigned long *LDA, double *B,
             const unsigned long *LDB, long *INFO);
/** Solve a triangular system (single precision) */
void strtrs_(const char *UPLO, const char *TRANS, const char *DIAG, const unsigned long *N,
             const unsigned long *NRHS, float *A, const unsigned long *LDA, float *B,
             const unsigned long *LDB, long *INFO);

/** Estimate the reciprocal of the condition number of a triangular matrix in double precision */
void dtrcon_(const char *NORM, const char *UPLO, const char *DIAG, const unsigned long *N,
             double *A, const unsigned long *LDA, double *RCOND, double *WORK,
             const unsigned long *IWORK, long *INFO);
/** Estimate the reciprocal of the condition number of a triangular matrix in single precision */
void strcon_(const char *NORM, const char *UPLO, const char *DIAG, const unsigned long *N, float *A,
             const unsigned long *LDA, float *RCOND, float *WORK, const unsigned long *IWORK,
             long *INFO);
}
...
class SQProblemSchur : public SQProblem {
 ...
 protected:
  ...
  virtual returnValue setupAuxiliaryQP(
      SymmetricMatrix
          *H_new, /**< New Hessian matrix. \n
                                                                                           If
                     Hessian matrix is trivial, a NULL pointer can be passed. */
      Matrix *A_new, /**< New constraint matrix. \n
                                              If QP sequence does not involve constraints, a NULL
   ...
   *	This must not be called for the Schur complement version. */
  virtual returnValue backsolveR(
      const real_t *const b, /**< Right hand side vector. */
      BooleanType transposed, /**< Indicates if the transposed system shall be solved. */
      real_t *const a /**< Output: Solution vector */
      ) const;
 ...
 private:
  ...
  returnValue addToSchurComplement(int_t number, SchurUpdateType update, int_t numNonzerosM,
                                   const sparse_int_t *M_pos, const real_t *const M_vals,
  ...
  /** Undo the last deletion from the Schur complement by moving the nS+1th row/column to position
   * idx. */
  ...
  /** Update QR factorization and determinant of Schur complement after a row and column have been
   * added or removed */
  returnValue updateSchurQR(int_t idxDel);

  /** Compute the solution to QRx = rhs and store it in sol */
  returnValue backsolveSchurQR(int_t dimS, const real_t *const rhs, int_t dimRhs,
                               real_t *const sol);

  /** If negative curvature is discovered in the reduced Hessian, add bounds until all eigenvalues
   * are positive */
  returnValue correctInertia();

  /** If the KKT matrix is declared singular during refactorization, remove linearly dependent
   * constraints or add bounds */
  returnValue repairSingularWorkingSet();

  returnValue stepCalcRhs(int_t nFR, int_t nFX, int_t nAC, int_t *FR_idx, int_t *FX_idx,
  ...
  returnValue stepCalcReorder(int_t nFR, int_t nAC, int_t *FR_idx, int_t *AC_idx, int_t nFRStart,
                              int_t nACStart, int_t *FR_idxStart, int_t *AC_idxStart,
                              int_t *FR_iSort, int_t *FR_iSortStart, int_t *AC_iSort,
                              int_t *AC_iSortStart, real_t *rhs);

  returnValue stepCalcBacksolveSchur(int_t nFR, int_t nFX, int_t nAC, int_t *FR_idx, int_t *FX_idx,
                                     int_t *AC_idx, int_t dim, real_t *rhs, real_t *sol);

  returnValue stepCalcReorder2(int_t nFR, int_t nAC, int_t *FR_idx, int_t *AC_idx, int_t nFRStart,
                               int_t nACStart, int_t *FR_idxStart, int_t *AC_idxStart,
                               int_t *FR_iSort, int_t *FR_iSortStart, int_t *AC_iSort,
                               int_t *AC_iSortStart, real_t *sol, real_t *const delta_xFR,
                               real_t *const delta_yAC);

  returnValue stepCalcResid(int_t nFR, int_t nFX, int_t nAC, int_t *FR_idx, int_t *FX_idx,
 ...
 protected:
  ...
  real_t
      *S; /**< Schur complement matrix. (This is actually the negative of the Schur complement!) */
  int_t nS; /**< Current size of Schur complement matrix. -1 means that the Schur complement has not
  ...
  int_t *schurUpdateIndex; /**< Indices of variables or constraints for each update in Schur
                              complement. */
  ...
  real_t *M_vals; /**< Values of the sparse M matrix containing the vectors with the additional rows
                     defining the Schur complement (length). */
  ...
  Indexlist constraintsActiveStart; /**< Index list for active constraints when major iteration
                                       started. */
};
