 ...
 *	\author Hans Joachim Ferreau, Andreas Potschka, Christian Kirches (thanks to Leonard
 *Wirsching) \version 3.2 \date 2007-2015
 *
...
enum returnValue {
  ...
  RET_NOT_YET_IMPLEMENTED, /**< Requested function is not yet implemented in this version of
                              qpOASES. */
  /* Indexlist */
  ...
  RET_INIT_FAILED_REGULARISATION, /**< Initialisation failed as Hessian matrix could not be
                                     regularised. */
  ...
  RET_NO_CHOLESKY_WITH_INITIAL_GUESS, /**< Externally computed Cholesky factor cannot be combined
  ...
  RET_HOTSTART_FAILED_AS_QP_NOT_INITIALISED, /**< Unable to perform homotopy as previous QP is not
                                                solved. */
  ...
  RET_CONSTRAINTS_NOT_SPECIFIED, /**< Problem does comprise constraints! You also have to specify
                                    new constraints' bounds. */
  ...
  RET_ADDCONSTRAINT_FAILED_INFEASIBILITY, /**< Addition of constraint to working set failed (due to
                                             QP infeasibility). */
  RET_ADDBOUND_FAILED, /**< Addition of bound to working set failed. */
  RET_ADDBOUND_FAILED_INFEASIBILITY, /**< Addition of bound to working set failed (due to QP
                                        infeasibility). */
  ...
  RET_CANNOT_REGULARISE_SPARSE, /**< Sparse matrix cannot be regularised as diagonal entry is
                                   missing. */
  RET_NO_REGSTEP_NWSR, /**< No additional regularisation step could be performed due to limits. */
  RET_FEWER_REGSTEPS_NWSR, /**< Fewer additional regularisation steps have been performed due to
                              limits. */
  RET_CHOLESKY_OF_ZERO_HESSIAN, /**< Cholesky decomposition of (unregularised) zero Hessian matrix.
                                 */
  RET_ZERO_HESSIAN_ASSUMED, /**< Zero Hessian matrix assumed as null pointer passed without
                               specifying hessianType. */
  ...
  RET_UPDATEMATRICES_FAILED_AS_QP_NOT_SOLVED, /**< Unable to update matrices as previous QP is not
                                                 solved. */
  /* Utils */
  ...
  RET_INERTIA_CORRECTION_FAILED, /**< Inertia correction failed after KKT matrix had too many
...
class MessageHandling {
 ...
 public:
   ...
   *  Errors are defined as abnormal events which cause an immediate termination of the current
   * (sub) function. Errors of a sub function should be commented by the calling function by means
   * of a warning message (if this error does not cause an error of the calling function, either)!
  ...
  returnValue throwError(
      ...
      VisibilityStatus
          localVisibilityStatus /**< Determines (locally) if error message can be printed to
                                 * stdFile. If GLOBAL visibility status of the message is set to
                                 * VS_HIDDEN, no message is printed, anyway! */
  );

  /** Prints a warning message (a simplified macro THROWWARNING is also provided).
   *  Warnings are definied as abnormal events which does NOT cause an immediate termination of the
   * current (sub) function. \return Warning number returned by sub function call
   */
  returnValue throwWarning(
      ...
      VisibilityStatus
          localVisibilityStatus /**< Determines (locally) if warning message can be printed to
                                 * stdFile. If GLOBAL visibility status of the message is set to
                                 * VS_HIDDEN, no message is printed, anyway! */
  );
  ...
  returnValue throwInfo(
      ...
      VisibilityStatus
          localVisibilityStatus /**< Determines (locally) if info message can be printed to stdFile.
 ...
 protected:
  ...
  returnValue throwMessage(
      ...
      VisibilityStatus
          localVisibilityStatus, /**< Determines (locally) if info message can be printed to
                                  * stdFile. If GLOBAL visibility status of the message is set to
                                  * VS_HIDDEN, no message is printed, anyway! */
...
#define THROWERROR(retval) \
  (getGlobalMessageHandler()->throwError((retval), 0, __FUNC__, __FILE__, __LINE__, VS_VISIBLE))

/** Short version of throwWarning with default values, only returnValue is needed */
#define THROWWARNING(retval) \
  (getGlobalMessageHandler()->throwWarning((retval), 0, __FUNC__, __FILE__, __LINE__, VS_VISIBLE))

/** Short version of throwInfo with default values, only returnValue is needed */
#define THROWINFO(retval) \
  (getGlobalMessageHandler()->throwInfo((retval), 0, __FUNC__, __FILE__, __LINE__, VS_VISIBLE))
