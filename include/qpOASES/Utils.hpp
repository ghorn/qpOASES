...
returnValue writeIntoFile(
    ...
    int_t ncol, /**< Column number of matrix. */
    const char *datafilename, /**< Data file name. */
    BooleanType append = BT_FALSE /**< Indicates if data shall be appended if the file already
...
returnValue writeIntoFile(
    ...
    int_t n, /**< Length of vector. */
    const char *datafilename, /**< Data file name. */
    BooleanType append = BT_FALSE /**< Indicates if integer shall be appended if the file already
                                     exists (otherwise it is overwritten). */
);
...
returnValue getKktViolation(
    ...
    const real_t
        *const H, /**< Hessian matrix (may be NULL if Hessian is zero or identity matrix). */
    ...
    const real_t *const workingSetC =
        0, /**< Working set of constraints (used to determine active constraints). */
    BooleanType hasIdentityHessian = BT_FALSE /**< Indicating whether Hessian matrix is identity
                                                 matrix or not if NULL pointer is passed. */
);
...
int_t getSimpleStatus(returnValue returnvalue, /**< ReturnValue to be analysed. */
                      BooleanType doPrintStatus = BT_FALSE /**< Flag indicating whether simple
                                                              status shall be printed to screen. */
);
