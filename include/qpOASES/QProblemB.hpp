...
class QProblemB {
 ...
 public:
   ...
   *	1. 0,    0,    0 : starts with xOpt = 0, yOpt = 0 and gB empty (or all implicit equality
  ...
  returnValue init(
      ...
      const real_t *const xOpt = 0, /**< Optimal primal solution vector. A NULL pointer can be
                                       passed. \n (If a null pointer is passed, the old primal
                                       solution is kept!) */
 ...
 protected:
  ...
  inline returnValue setH(
      SymmetricMatrix *H_new /**< New Hessian matrix (a shallow copy is made). */
  );
   ...
   *	\return SUCCESSFUL_RETURN */
  inline returnValue setH(const real_t *const H_new /**< New dense Hessian matrix (with correct
  ...
  returnValue performRatioTest(
      ...
      const SubjectTo
          *const subjectTo, /**< Bound/Constraint object corresponding to ratios to be checked. */
      const real_t
 ...
 private:
  ...
  /** Removes a bounds from active set (specialised version for the case where no constraints
   exist). *	\return SUCCESSFUL_RETURN \n RET_HESSIAN_NOT_SPD \n RET_REMOVEBOUND_FAILED */
 ...
 protected:
  BooleanType
      freeHessian; /**< Flag indicating whether the Hessian matrix needs to be de-allocated. */
  ...
  BooleanType
      haveCholesky; /**< Flag indicating whether Cholesky decomposition has already been setup. */
  ...
  TabularOutput tabularOutput; /**< Struct storing information for tabular output (printLevel ==
                                  PL_TABULAR). */
};
