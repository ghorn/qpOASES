...
MessageHandling::ReturnValueList returnValueList[] = {
    ...
    {RET_INIT_FAILED_REGULARISATION,
     "Initialisation failed as Hessian matrix could not be regularised", VS_VISIBLE},
    ...
    {RET_NO_CHOLESKY_WITH_INITIAL_GUESS,
     "Externally computed Cholesky factor cannot be combined with an initial guess", VS_VISIBLE},
    ...
    {RET_CONSTRAINTS_NOT_SPECIFIED,
     "Problem does comprise constraints! You have to specify new constraints' bounds", VS_VISIBLE},
    ...
    {RET_CANNOT_REGULARISE_SPARSE,
     "Sparse matrix cannot be regularised as diagonal entry is missing", VS_VISIBLE},
 ...
 *	M e s s a g e H a n d l i n g
 */
MessageHandling::MessageHandling(FILE *_outputFile, VisibilityStatus _errorVisibility,
                                 VisibilityStatus _warningVisibility,
                                 VisibilityStatus _infoVisibility) {
  errorVisibility = _errorVisibility;
  warningVisibility = _warningVisibility;
 ...
 *	t h r o w E r r o r
 */
returnValue MessageHandling::throwError(returnValue Enumber, const char *additionaltext,
  ...
  if (errorVisibility == VS_VISIBLE)
    return throwMessage(Enumber, additionaltext, functionname, filename, linenumber,
                        localVisibilityStatus, "ERROR");
  else
    return Enumber;
}
 ...
 */
returnValue MessageHandling::throwWarning(returnValue Wnumber, const char *additionaltext,
                                          const char *functionname, const char *filename,
                                          const unsigned long linenumber,
                                          VisibilityStatus localVisibilityStatus) {
  /* consistency check */
  ...
  if (warningVisibility == VS_VISIBLE)
    return throwMessage(Wnumber, additionaltext, functionname, filename, linenumber,
                        localVisibilityStatus, "WARNING");
  else
    return Wnumber;
}
 ...
 *	t h r o w I n f o
 */
returnValue MessageHandling::throwInfo(returnValue Inumber, const char *additionaltext,
                                       const char *functionname, const char *filename,
                                       const unsigned long linenumber,
                                       VisibilityStatus localVisibilityStatus) {
  /* consistency check */
  ...
  if (infoVisibility == VS_VISIBLE)
    return throwMessage(Inumber, additionaltext, functionname, filename, linenumber,
                        localVisibilityStatus, "INFO");
  else
    return Inumber;
}
 ...
 */
returnValue MessageHandling::throwMessage(returnValue RETnumber, const char *additionaltext,
                                          const char *functionname, const char *filename,
                                          const unsigned long linenumber,
                                          VisibilityStatus localVisibilityStatus,
  ...
  if ((returnValueList[keypos].globalVisibilityStatus == VS_VISIBLE) &&
    ...
    if (additionaltext == 0) {
      ...
      snprintf(myPrintfString, MAX_STRING_LENGTH, "%s (%s, %s:%d): \t%s\n", RETstring, functionname,
               filename, (int_t)linenumber, returnValueList[keypos].data);
#else
      ...
      snprintf(myPrintfString, MAX_STRING_LENGTH, "%s (%s, %s:%d): \t%s %s\n", RETstring,
               functionname, filename, (int_t)linenumber, returnValueList[keypos].data,
               additionaltext);
#else
