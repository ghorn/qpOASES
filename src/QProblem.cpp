...
returnValue QProblem::init(SymmetricMatrix *_H, const real_t *const _g, Matrix *_A,
                           ...
                           const real_t *const _lbA, const real_t *const _ubA, int_t &nWSR,
                           real_t *const cputime, const real_t *const xOpt,
                           const real_t *const yOpt, const Bounds *const guessedBounds,
                           const Constraints *const guessedConstraints, const real_t *const _R) {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  if ((_R != 0) &&
      ((xOpt != 0) || (yOpt != 0) || (guessedBounds != 0) || (guessedConstraints != 0)))
...
returnValue QProblem::init(const char *const H_file, const char *const g_file,
                           ...
                           const char *const ubA_file, int_t &nWSR, real_t *const cputime,
                           const real_t *const xOpt, const real_t *const yOpt,
                           const Bounds *const guessedBounds,
                           const Constraints *const guessedConstraints, const char *const R_file) {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  if (setupQPdataFromFile(H_file, g_file, A_file, lb_file, ub_file, lbA_file, ubA_file) !=
      SUCCESSFUL_RETURN)
 ...
 *	h o t s t a r t
 */
returnValue QProblem::hotstart(const real_t *const g_new, const real_t *const lb_new,
  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    returnvalue =
        solveRegularisedQP(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime, 0, isFirstCall);
  } else {
    ...
    updateFarBounds(farbound, nV + nC, lb_new, lb_new_far, ub_new, ub_new_far, lbA_new, lbA_new_far,
                    ubA_new, ubA_new_far);
    ...
    for (;;) {
      ...
      returnvalue = solveRegularisedQP(g_new, lb_new_far, ub_new_far, lbA_new_far, ubA_new_far,
                                       nWSR, pcputime_rem, nWSR_performed, isFirstCall);
      ...
      if (infeasible == BT_TRUE) {
        ...
        updateFarBounds(farbound, nV + nC, lb_new, lb_new_far, ub_new, ub_new_far, lbA_new,
                        lbA_new_far, ubA_new, ubA_new_far);
 ...
 *	h o t s t a r t
 */
returnValue QProblem::hotstart(const char *const g_file, const char *const lb_file,
                               const char *const ub_file, const char *const lbA_file,
                               const char *const ubA_file, int_t &nWSR, real_t *const cputime,
                               const Bounds *const guessedBounds,
                               const Constraints *const guessedConstraints) {
  ...
  returnvalue = loadQPvectorsFromFile(g_file, lb_file, ub_file, lbA_file, ubA_file, g_new, lb_new,
  ...
  returnvalue = hotstart(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime, guessedBounds,
                         guessedConstraints);
 ...
 */
returnValue QProblem::solveCurrentEQP(const int_t n_rhs, const real_t *g_in, const real_t *lb_in,
                                      const real_t *ub_in, const real_t *lbA_in,
                                      const real_t *ubA_in, real_t *x_out, real_t *y_out) {
  if ((x_out == 0) || (y_out == 0))
    return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  for (ii = 0; ii < n_rhs; ++ii) {
    returnvalue = determineStepDirection(g_in, lbA_in, ubA_in, lb_in, ub_in, BT_FALSE, BT_FALSE,
...
returnValue QProblem::printProperties() {
  ...
  switch (hessianType) {
    case HST_ZERO: myPrintf("Hessian is zero matrix (i.e. actually an LP is solved).\n"); break;

    case HST_IDENTITY: myPrintf("Hessian is identity matrix.\n"); break;

    case HST_POSDEF: myPrintf("Hessian matrix is (strictly) positive definite.\n"); break;

    case HST_POSDEF_NULLSPACE:
      myPrintf("Hessian matrix is positive definite on null space of active constraints.\n");
      break;

    case HST_SEMIDEF: myPrintf("Hessian matrix is positive semi-definite.\n"); break;

    case HST_INDEF: myPrintf("Hessian matrix is indefinite.\n"); break;

    default: myPrintf("Hessian matrix has unknown type.\n"); break;
  }
  ...
  switch (status) {
    ...
    case QPS_SOLVED: myPrintf("Status of QP object: solution of the actual QP was found.\n"); break;
  }
  ...
  switch (options.printLevel) {
    ...
    case PL_LOW: myPrintf("Print level of QP object is low, i.e. only error are printed.\n"); break;
...
returnValue QProblem::solveInitialQP(const real_t *const xOpt, const real_t *const yOpt,
                                     const Bounds *const guessedBounds,
                                     const Constraints *const guessedConstraints,
                                     const real_t *const _R, int_t &nWSR, real_t *const cputime) {
  int_t i, j;
 ...
 *	s o l v e Q P
 */
returnValue QProblem::solveQP(const real_t *const g_new, const real_t *const lb_new,
                              const real_t *const ub_new, const real_t *const lbA_new,
                              const real_t *const ubA_new, int_t &nWSR, real_t *const cputime,
  ...
  status = QPS_PERFORMINGHOMOTOPY;  // AW TODO: Not sure if this is too early, but otherwise
                                    // removeBounds will complain
  ...
  */
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    tabularOutput.idxAddB = tabularOutput.idxRemB = tabularOutput.idxAddC = tabularOutput.idxRemC =
        -1;
    tabularOutput.excAddB = tabularOutput.excRemB = tabularOutput.excAddC = tabularOutput.excRemC =
        0;
    ...
    if (isFirstCall == BT_TRUE)
      ...
      snprintf(messageString, MAX_STRING_LENGTH, "%d* ...", (int)iter);
    getGlobalMessageHandler()->throwInfo(RET_ITERATION_STARTED, messageString, __FUNC__, __FILE__,
                                         __LINE__, VS_VISIBLE);
#endif

    /* 2) Determination of shift direction of the gradient and the (constraints') bounds. */
    returnvalue =
        determineDataShift(g_new, lbA_new, ubA_new, lb_new, ub_new, delta_g, delta_lbA, delta_ubA,
    ...
    returnvalue =
        determineStepDirection(delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, Delta_bC_isZero,
    ...
    returnvalue = performStep(delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, delta_xFX,
                              delta_xFR, delta_yAC, delta_yFX, BC_idx, BC_status, BC_isBound);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yAC;
      delete[] delta_yFX;
      delete[] delta_xFX;
    ...
    if (homotopyLength <= options.terminationTolerance) {
      ...
      if (printIteration(iter, BC_idx, BC_status, BC_isBound, homotopyLength, isFirstCall) !=
          SUCCESSFUL_RETURN)
  ...
  if (options.printLevel == PL_HIGH) {
    ...
    return getGlobalMessageHandler()->throwWarning(RET_MAX_NWSR_REACHED, messageString, __FUNC__,
 ...
 */
returnValue QProblem::solveRegularisedQP(const real_t *const g_new, const real_t *const lb_new,
                                         const real_t *const ub_new, const real_t *const lbA_new,
  ...
  if (cputime == 0) {
    ...
    cputime_cur = *cputime;
    returnvalue = solveQP(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, &cputime_cur,
  ...
  if ((lbA_new != 0) && (ubA_new != 0)) {
    for (i = 0; i < nC; ++i) {
      ...
      if ((lbA_new[i] < -INFTY + options.boundTolerance) &&
          (ubA_new[i] > INFTY - options.boundTolerance) && (options.enableFarBounds == BT_FALSE)) {
  ...
  if (info > 0) {
    if (R[0] < 0.0) {
      ...
      options.epsRegularisation =
          getMin(-R[0] + options.epsRegularisation, getSqrt(getAbs(options.epsRegularisation)));
    }
...
returnValue QProblem::setupAuxiliaryWorkingSet(const Bounds *const auxiliaryBounds,
  ...
  if (setupAfresh == BT_FALSE) {
    ...
    for (i = 0; i < nC; ++i) {
      if ((constraints.getStatus(i) == ST_LOWER) &&
        ...
        if (removeConstraint(i, updateCholesky, BT_FALSE, options.enableNZCTests) !=
            SUCCESSFUL_RETURN)
  ...
  for (i = 0; i < nC; ++i) {
    // if ( ( constraints.getType( i ) == ST_EQUALITY ) && ( ( constraints.getStatus( i ) ==
    // ST_INACTIVE ) && ( auxiliaryConstraints->getStatus( i ) != ST_INACTIVE ) ) )
  ...
  for (i = 0; i < nV; ++i) {
    if ((bounds.getType(i) != ST_EQUALITY) &&
        ((bounds.getStatus(i) == ST_INACTIVE) && (auxiliaryBounds->getStatus(i) != ST_INACTIVE))) {
...
returnValue QProblem::setupAuxiliaryQPbounds(const Bounds *const auxiliaryBounds,
  ...
  for (i = 0; i < nV; ++i) {
    switch (bounds.getStatus(i)) {
      ...
      case ST_DISABLED: break;

      default: return THROWERROR(RET_UNKNOWN_BUG);
    }
  }
...
returnValue QProblem::addConstraint(int_t number, SubjectToStatus C_status,
                                    BooleanType updateCholesky, BooleanType ensureLI) {
  int_t i, j, ii;
  ...
  if (updateCholesky == BT_TRUE && ensureLI == BT_TRUE) {
    ...
    switch (ensureLIreturnvalue) {
      ...
      case RET_LI_RESOLVED: break;

      case RET_ENSURELI_FAILED_NOINDEX: return RET_ADDCONSTRAINT_FAILED_INFEASIBILITY;

      case RET_ENSURELI_FAILED_CYCLING: return RET_ADDCONSTRAINT_FAILED_INFEASIBILITY;

      case RET_ENSURELI_DROPPED: return SUCCESSFUL_RETURN;

      default: return THROWERROR(RET_ENSURELI_FAILED);
    }
  }
  ...
  if (nZ > 0) {
    ...
    if ((updateCholesky == BT_TRUE) && (hessianType != HST_ZERO) && (hessianType != HST_IDENTITY)) {
...
returnValue QProblem::addConstraint_checkLI(int_t number) {
  ...
  if (options.enableFullLITests) {
    ...
    returnValue dsdreturnvalue =
        determineStepDirection(delta_g, nul, nul, nul, nul, BT_FALSE, BT_FALSE, delta_xFX,
...
returnValue QProblem::addConstraint_ensureLI(int_t number, SubjectToStatus C_status) {
  ...
  performRatioTest(nAC, AC_idx, &constraints, num, xiC, options.epsNum, options.epsDen, y_min,
                   y_min_number);
  ...
  performRatioTest(nFX, FX_idx, &bounds, num, xiB, options.epsNum, options.epsDen, y_min,
                   y_min_number_bound);
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound == BT_TRUE) {
      ...
      getGlobalMessageHandler()->throwInfo(RET_REMOVE_FROM_ACTIVESET, messageString, __FUNC__,
                                           __FILE__, __LINE__, VS_VISIBLE);
#endif
...
farewell:
  ...
  getGlobalMessageHandler()->throwInfo(RET_LI_RESOLVED, 0, __FUNC__, __FILE__, __LINE__,
                                       VS_VISIBLE);

  return ((returnvalue != SUCCESSFUL_RETURN) && (returnvalue != RET_ENSURELI_FAILED_NOINDEX))
 ...
 *	a d d B o u n d
 */
returnValue QProblem::addBound(int_t number, SubjectToStatus B_status, BooleanType updateCholesky,
  ...
  if ((updateCholesky == BT_TRUE) && (ensureLI == BT_TRUE)) {
    ...
    switch (ensureLIreturnvalue) {
      ...
      case RET_ENSURELI_FAILED_NOINDEX: return RET_ADDBOUND_FAILED_INFEASIBILITY;

      case RET_ENSURELI_FAILED_CYCLING: return RET_ADDBOUND_FAILED_INFEASIBILITY;
...
returnValue QProblem::removeConstraint(int_t number, BooleanType updateCholesky,
                                       BooleanType allowFlipping, BooleanType ensureNZC) {
  int_t i, j, ii, jj;
  returnValue returnvalue = SUCCESSFUL_RETURN;
  ...
  if (ensureNZC == BT_TRUE) {
    returnvalue = ensureNonzeroCurvature(BT_FALSE, number, exchangeHappened, addBoundNotConstraint,
                                         addIdx, addStatus);
  ...
  if ((exchangeHappened == BT_FALSE) && (options.enableFlippingBounds == BT_TRUE) &&
      (allowFlipping == BT_TRUE))
  ...
  if (number_idx < nAC - 1) {
    ...
    for (j = (nAC - 2 - number_idx); j >= 0; --j) {
      computeGivens(TT(nAC - 2 - j, tcol + 1 + j), TT(nAC - 2 - j, tcol + j),
                    TT(nAC - 2 - j, tcol + 1 + j), TT(nAC - 2 - j, tcol + j), c, s);
      nu = s / (1.0 + c);

      for (i = (nAC - j - 1); i < (nAC - 1); ++i)
        applyGivens(c, s, nu, TT(i, tcol + 1 + j), TT(i, tcol + j), TT(i, tcol + 1 + j),
                    TT(i, tcol + j));

      for (i = 0; i < nFR; ++i) {
        ii = FR_idx[i];
        applyGivens(c, s, nu, QQ(ii, nZ + 1 + j), QQ(ii, nZ + j), QQ(ii, nZ + 1 + j),
                    QQ(ii, nZ + j));
      }
    }
  } else {
...
returnValue QProblem::removeBound(int_t number, BooleanType updateCholesky,
  ...
  if (ensureNZC == BT_TRUE) {
    returnvalue = ensureNonzeroCurvature(BT_TRUE, number, exchangeHappened, addBoundNotConstraint,
                                         addIdx, addStatus);
  ...
  if (nAC > 0) {
    /* store new column a in a temporary vector instead of shifting T one column to the left and
     * appending a */
    int_t *AC_idx;
    constraints.getActive()->getNumberArray(&AC_idx);
    ...
    for (j = (nAC - 1); j >= 0; --j) {
      computeGivens(tmp[nAC - 1 - j], TT(nAC - 1 - j, tcol + j), TT(nAC - 1 - j, tcol + j),
                    tmp[nAC - 1 - j], c, s);
      nu = s / (1.0 + c);
...
returnValue QProblem::performPlainRatioTest(int_t nIdx, const int_t *const idxList,
                                            const real_t *const num, const real_t *const den,
                                            real_t epsNum, real_t epsDen, real_t &t,
                                            int_t &BC_idx) const {
  int_t i;
  for (i = 0; i < nIdx; i++)
...
returnValue QProblem::ensureNonzeroCurvature(BooleanType removeBoundNotConstraint, int_t remIdx,
                                             BooleanType &exchangeHappened,
                                             BooleanType &addBoundNotConstraint, int_t &addIdx,
                                             SubjectToStatus &addStatus) {
  int_t i, ii;
  ...
  if (removeBoundNotConstraint) {
    ...
    returnvalue = determineStepDirection(nul, nul, nul, ek, ek, BT_FALSE, BT_FALSE, delta_xFX,
                                         delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  } else {
    ...
    returnvalue = determineStepDirection(nul, ek, ek, nul, nul, BT_FALSE, BT_TRUE, delta_xFX,
                                         delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  }
  ...
  if (normXi < options.epsNZCTests * normS) {
    ...
    /* performRatioTest( nFR,FR_idx,&bounds, x_W,delta_xFR, options.epsNum,options.epsDen,
     * sigmaUBnd,addUBndIdx ); */
    sigmaUBnd = options.maxPrimalJump;
    addUBndIdx = -1;
    performPlainRatioTest(nFR, FR_idx, x_W, delta_xFR, options.epsNum, options.epsDen, sigmaUBnd,
                          addUBndIdx);
    if (removeBoundNotConstraint == BT_TRUE && bounds.getStatus(remIdx) == ST_LOWER) {
      ...
      performPlainRatioTest(1, &remIdx, x_W, &one, options.epsNum, options.epsDen, sigmaUBnd,
                            addUBndIdx);
    }
    ...
    /* performRatioTest( nFR,FR_idx,&bounds, x_W,delta_xFR, options.epsNum,options.epsDen,
     * sigmaLBnd,addLBndIdx ); */
    sigmaLBnd = options.maxPrimalJump;
    addLBndIdx = -1;
    performPlainRatioTest(nFR, FR_idx, x_W, delta_xFR, options.epsNum, options.epsDen, sigmaLBnd,
                          addLBndIdx);
    if (removeBoundNotConstraint == BT_TRUE && bounds.getStatus(remIdx) == ST_UPPER) {
      /* also consider bound which is to be removed */
      real_t one = 1.0;
      x_W[0] = x[remIdx] - lb[remIdx];
      performPlainRatioTest(1, &remIdx, x_W, &one, options.epsNum, options.epsDen, sigmaLBnd,
                            addLBndIdx);
    }
    for (i = 0; i < nFR; i++)
      delta_xFR[i] = -delta_xFR[i];
    ...
    /* performRatioTest( nIAC,IAC_idx,&constraints, Ax_W,As, options.epsNum,options.epsDen,
     * sigmaUCnstr,addUCnstrIdx ); */
    sigmaUCnstr = options.maxPrimalJump;
    addUCnstrIdx = -1;
    performPlainRatioTest(nIAC, IAC_idx, Ax_W, As, options.epsNum, options.epsDen, sigmaUCnstr,
                          addUCnstrIdx);
    if (removeBoundNotConstraint == BT_FALSE && constraints.getStatus(remIdx) == ST_LOWER) {
      ...
      real_t one = 1.0;
      performPlainRatioTest(1, &remIdx, &Ax_u[remIdx], &one, options.epsNum, options.epsDen,
                            sigmaUCnstr, addUCnstrIdx);
    }
    ...
    for (i = 0; i < nIAC; i++)
      As[i] = -As[i];
    /* performRatioTest( nIAC,IAC_idx,&constraints, Ax_W,As, options.epsNum,options.epsDen,
     * sigmaLCnstr,addLCnstrIdx ); */
    sigmaLCnstr = options.maxPrimalJump;
    addLCnstrIdx = -1;
    performPlainRatioTest(nIAC, IAC_idx, Ax_W, As, options.epsNum, options.epsDen, sigmaLCnstr,
                          addLCnstrIdx);
    if (removeBoundNotConstraint == BT_FALSE && constraints.getStatus(remIdx) == ST_UPPER) {
      /* also consider constraint which is to be removed */
      real_t one = 1.0;
      performPlainRatioTest(1, &remIdx, &Ax_l[remIdx], &one, options.epsNum, options.epsDen,
                            sigmaLCnstr, addLCnstrIdx);
    }
 ...
 */
returnValue QProblem::determineDataShift(const real_t *const g_new, const real_t *const lbA_new,
  ...
  QProblemB::determineDataShift(g_new, lb_new, ub_new, delta_g, delta_lb, delta_ub,
                                Delta_bB_isZero);
...
returnValue QProblem::determineStepDirection(
    ...
    const real_t *const delta_lb, const real_t *const delta_ub, BooleanType Delta_bC_isZero,
    BooleanType Delta_bB_isZero, real_t *const delta_xFX, real_t *const delta_xFR,
  ...
  for (r = 0; r <= options.numRefinementSteps; ++r) {
      ...
      if (nAC > 0) {
        if ((Delta_bC_isZero == BT_TRUE) && (Delta_bB_isZero == BT_TRUE)) {
          ...
          if ((Delta_bB_isZero == BT_FALSE) && (r == 0))
            A->times(constraints.getActive(), bounds.getFixed(), 1, -1.0, delta_xFX, nFX, 1.0,
                     tempB, nAC);
    ...
    if (nAC > 0) /* => ( nFR = nZ + nAC > 0 ) */
    {
      if ((hessianType == HST_ZERO) || (hessianType == HST_IDENTITY)) {
        ...
        if (nZ > 0)
          H->times(bounds.getFree(), bounds.getFree(), 1, 1.0, ZFR_delta_xFRz, nFR, 1.0, tempA,
                   nFR);
    ...
    if (options.numRefinementSteps > 0) {
      ...
      A->transTimes(constraints.getActive(), bounds.getFree(), 1, -1.0, delta_yAC, nAC, 1.0, tempA,
                    nFR);
      real_t rnrm = 0.0;
      for (i = 0; i < nFR; ++i)
      ...
      A->times(constraints.getActive(), bounds.getFixed(), 1, -1.0, delta_xFX, nFX, 1.0, tempB,
               nAC);
      for (i = 0; i < nAC; ++i)
  ...
  if (nFX > 0) {
    ...
    A->transTimes(constraints.getActive(), bounds.getFixed(), 1, -1.0, delta_yAC, nAC, 1.0,
                  delta_yFX, nFX);
 ...
 */
returnValue QProblem::performStep(const real_t *const delta_g, const real_t *const delta_lbA,
  ...
  performRatioTest(nAC, AC_idx, &constraints, num, den, options.epsNum, options.epsDen, tau,
                   BC_idx_tmp);
  ...
  if (constraints.hasNoLower() == BT_FALSE) {
    ...
    performRatioTest(nIAC, IAC_idx, &constraints, num, den, options.epsNum, options.epsDen, tau,
                     BC_idx_tmp);
  ...
  if (bounds.hasNoLower() == BT_FALSE) {
    ...
    performRatioTest(nFR, FR_idx, &bounds, num, den, options.epsNum, options.epsDen, tau,
                     BC_idx_tmp);
  ...
  if (BC_status == ST_UNDEFINED)
    ...
    snprintf(messageString, MAX_STRING_LENGTH,
             "Stepsize is %.15e! (idx = %d, isBound = %d, status = %d)", tau, (int)BC_idx,
             (int)BC_isBound, (int)BC_status);

  getGlobalMessageHandler()->throwInfo(RET_STEPSIZE_NONPOSITIVE, messageString, __FUNC__, __FILE__,
                                       __LINE__, VS_VISIBLE);
#endif
...
#ifndef __SUPPRESSANYOUTPUT__
    snprintf(messageString, MAX_STRING_LENGTH, "Stepsize is %.15e", tau);
    getGlobalMessageHandler()->throwWarning(RET_STEPSIZE, messageString, __FUNC__, __FILE__,
                                            __LINE__, VS_VISIBLE);
#endif
  }
...
returnValue QProblem::changeActiveSet(int_t BC_idx, SubjectToStatus BC_status,
  ...
  switch (BC_status) {
    ...
    case ST_UNDEFINED: return SUCCESSFUL_RETURN;
    ...
    default:
      ...
      if (BC_isBound == BT_TRUE) {
        ...
        getGlobalMessageHandler()->throwInfo(RET_ADD_TO_ACTIVESET, messageString, __FUNC__,
                                             __FILE__, __LINE__, VS_VISIBLE);
#endif
...
#ifndef __SUPPRESSANYOUTPUT__
        if (BC_status == ST_LOWER)
          snprintf(messageString, MAX_STRING_LENGTH, "lower constraint's bound no. %d.",
                   (int)BC_idx);
        else
          snprintf(messageString, MAX_STRING_LENGTH, "upper constraint's bound no. %d.",
                   (int)BC_idx);
        getGlobalMessageHandler()->throwInfo(RET_ADD_TO_ACTIVESET, messageString, __FUNC__,
                                             __FILE__, __LINE__, VS_VISIBLE);
#endif
 ...
 */
real_t QProblem::getRelativeHomotopyLength(const real_t *const g_new, const real_t *const lb_new,
                                           const real_t *const ub_new, const real_t *const lbA_new,
                                           const real_t *const ubA_new) {
  int_t i;
  int_t nC = getNC();
 ...
 */
returnValue QProblem::updateFarBounds(real_t curFarBound, int_t nRamp, const real_t *const lb_new,
  ...
  returnValue returnvalue =
      QProblemB::updateFarBounds(curFarBound, nRamp, lb_new, lb_new_far, ub_new, ub_new_far);
  if (returnvalue != SUCCESSFUL_RETURN)
    return returnvalue;
    ...
    switch (bounds.getType(i)) {
      case ST_BOUNDED:
        switch (bounds.getStatus(i)) {
          ...
          case ST_INFEASIBLE_UPPER: break;
        }
        break;
      case ST_EQUALITY:
        lb[i] = x[i];
 ...
 *	s e t u p Q P d a t a
 */
returnValue QProblem::setupQPdata(const real_t *const _H, const real_t *const _g,
 ...
 */
returnValue QProblem::setupQPdataFromFile(const char *const H_file, const char *const g_file,
                                          const char *const A_file, const char *const lb_file,
                                          const char *const ub_file, const char *const lbA_file,
 ...
 */
returnValue QProblem::loadQPvectorsFromFile(const char *const g_file, const char *const lb_file,
                                            const char *const ub_file, const char *const lbA_file,
                                            const char *const ubA_file, real_t *const g_new,
                                            real_t *const lb_new, real_t *const ub_new,
                                            real_t *const lbA_new, real_t *const ubA_new) const {
  int_t nC = getNC();
 ...
 */
returnValue QProblem::printIteration(int_t iter, int_t BC_idx, SubjectToStatus BC_status,
                                     BooleanType BC_isBound, real_t homotopyLength,
                                     BooleanType isFirstCall) {
#ifndef __SUPPRESSANYOUTPUT__
  ...
  switch (options.printLevel) {
    case PL_DEBUG_ITER:
      ...
      switch (hessianType) {
        ...
        default: H->times(1, 1.0, x, nV, 1.0, grad, nV); break;
      }
      ...
      if ((iter % 10 == 0) && (isFirstCall == BT_TRUE)) {
        snprintf(myPrintfString, MAX_STRING_LENGTH,
                 ...
                 "addC", "remC", "hom len", "tau", "stat", "bfeas", "cfeas", "bcmpl", "ccmpl",
                 "Tmin");
        myPrintf(myPrintfString);
      }
    ...
    case PL_TABULAR:
      if ((iter % 10 == 0) && (isFirstCall == BT_TRUE)) {
        snprintf(myPrintfString, MAX_STRING_LENGTH, "\n%5s %6s %6s %6s %6s %9s %9s\n", "iter",
                 "addB", "remB", "addC", "remC", "hom len", "tau");
        myPrintf(myPrintfString);
      }
      if (isFirstCall == BT_TRUE)
        snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d ", (int)iter);
      ...
      if (tabularOutput.idxAddB >= 0) {
        snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxAddB),
                 excStr[tabularOutput.excAddB]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
      ...
      if (tabularOutput.idxRemB >= 0) {
        snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxRemB),
                 excStr[tabularOutput.excRemB]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
      ...
      if (tabularOutput.idxAddC >= 0) {
        snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxAddC),
                 excStr[tabularOutput.excAddC]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
      ...
      if (tabularOutput.idxRemC >= 0) {
        snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ", (int)(tabularOutput.idxRemC),
                 excStr[tabularOutput.excRemC]);
        myPrintf(myPrintfString);
      } else {
        myPrintf("       ");
      }
      ...
      if (BC_status == ST_UNDEFINED) {
        ...
        if (isFirstCall == BT_TRUE)
          snprintf(myPrintfString, MAX_STRING_LENGTH,
                   ...
                   tau, info, (int)getNFX(), (int)getNAC());
        else
          snprintf(myPrintfString, MAX_STRING_LENGTH,
                   "   %5.1d*  |   %1.6e   |    %s SOLVED     |  %4.1d   |  %4.1d   \n", (int)iter,
                   tau, info, (int)getNFX(), (int)getNAC());
        ...
        snprintf(myPrintfString, MAX_STRING_LENGTH,
                 ...
                 info, (int)BC_idx, (int)getNFX(), (int)getNAC());
        myPrintf(myPrintfString);
      }
      break;
 ...
 */
returnValue QProblem::dropInfeasibles(int_t BC_number, SubjectToStatus BC_status,
                                      BooleanType BC_isBound, real_t *xiB, real_t *xiC) {
  int_t i;
  ...
  if (options.dropEqConPriority <= y_min_priority) {
    ...
    for (i = 0; i < nAC; ++i)
      if ((constraints.getType(i) == ST_EQUALITY) && (getAbs(xiC[i]) > options.epsDen)) {
  ...
  if (options.dropIneqConPriority <= y_min_priority) {
    ...
    for (i = 0; i < nAC; ++i)
      if ((constraints.getType(i) == ST_BOUNDED) && (getAbs(xiC[i]) > options.epsDen)) {
        y_min_number = AC_idx[i];
        y_min_isBound = BT_FALSE;
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound) {
      ...
      bounds.setStatus(y_min_number,
                       (status_ == ST_LOWER) ? ST_INFEASIBLE_LOWER : ST_INFEASIBLE_UPPER);
      // TODO: fix duals y[]
      ...
      constraints.setStatus(y_min_number,
                            (status_ == ST_LOWER) ? ST_INFEASIBLE_LOWER : ST_INFEASIBLE_UPPER);
      // TODO: fix duals y[]
    ...
    // ... now return, add the blocking constraint, and continue solving QP with dropped
    // bound/constraint
    return SUCCESSFUL_RETURN;
    ...
    if (BC_isBound)
      bounds.setStatus(BC_number,
                       (BC_status == ST_LOWER) ? ST_INFEASIBLE_LOWER : ST_INFEASIBLE_UPPER);
    else
      constraints.setStatus(BC_number,
                            (BC_status == ST_LOWER) ? ST_INFEASIBLE_LOWER : ST_INFEASIBLE_UPPER);
