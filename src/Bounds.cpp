...
returnValue Bounds::setupBound(int_t number, SubjectToStatus _status) {
  ...
  switch (_status) {
    ...
    default: return THROWERROR(RET_INVALID_ARGUMENTS);
  }
...
returnValue Bounds::shift(int_t offset) {
  ...
  for (i = 0; i < n; ++i) {
    switch (getStatus(i)) {
      ...
      default: return THROWERROR(RET_SHIFTING_FAILED);
    }
  }
...
returnValue Bounds::rotate(int_t offset) {
  ...
  for (i = 0; i < n; ++i) {
    switch (getStatus(i)) {
      ...
      default: return THROWERROR(RET_ROTATING_FAILED);
    }
  }
...
returnValue Bounds::setupAll(SubjectToStatus _status) {
  ...
  /* 2) Add remaining (i.e. bounded but possibly free) variables to the index list of free
   * variables. */
  for (i = 0; i < n; ++i) {
