 ...
 */
real_t SolutionAnalysis::getKktViolation(QProblemB *const qp, real_t *const maxStat,
                                         real_t *const maxFeas, real_t *const maxCmpl) const {
  int_t i;
  int_t nV = qp->getNV();
  ...
  switch (qp->getHessianType()) {
    ...
    case HST_IDENTITY: hasIdentityHessian = BT_TRUE; break;
  ...
  returnValue returnvalue =
      REFER_NAMESPACE_QPOASES getKktViolation(nV, H_ptr, qp->g, qp->lb, qp->ub, qp->x, qp->y, stat,
                                              feas, cmpl, workingSetB, hasIdentityHessian);
  if (workingSetB != 0)
    delete[] workingSetB;
 ...
 *	g e t K k t V i o l a t i o n
 */
real_t SolutionAnalysis::getKktViolation(QProblem *const qp, real_t *const maxStat,
                                         real_t *const maxFeas, real_t *const maxCmpl) const {
  int_t i;
  int_t nV = qp->getNV();
  int_t nC = qp->getNC();
  ...
  returnValue returnvalue = REFER_NAMESPACE_QPOASES getKktViolation(
      nV, nC, H_ptr, qp->g, A_ptr, qp->lb, qp->ub, qp->lbA, qp->ubA, qp->x, qp->y, stat, feas, cmpl,
 ...
 *	g e t K k t V i o l a t i o n
 */
real_t SolutionAnalysis::getKktViolation(SQProblem *const qp, real_t *const maxStat,
                                         real_t *const maxFeas, real_t *const maxCmpl) const {
 ...
 *	g e t V a r i a n c e C o v a r i a n c e
 */
returnValue SolutionAnalysis::getVarianceCovariance(QProblem *qp, const real_t *const g_b_bA_VAR,
                                                    real_t *const Primal_Dual_VAR) const {
  ...
  for (run3 = 0; run3 < dim; run3++) {
    ...
    returnvalue = qp->determineStepDirection(
        delta_g_cov, delta_lbA_cov, delta_ubA_cov, delta_lb_cov, delta_ub_cov, Delta_bC_isZero,
...
returnValue SolutionAnalysis::checkCurvatureOnStronglyActiveConstraints(SQProblem *qp) {
  printf(
      "checkCurvatureOnStronglyActiveConstraints( SQProblem* qp ) not yet implemented for standard "
      "qpOASES!\n");
...
returnValue SolutionAnalysis::checkCurvatureOnStronglyActiveConstraints(SQProblemSchur *qp) {
  ...
  // If no bounds are active reduced Hessian is positive definite (otherwise qpOASES wouldnt have
  // finished)
  if (nFX == 0)
    return SUCCESSFUL_RETURN;
