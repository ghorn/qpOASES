 ...
 *	\author Andreas Waechter and Dennis Janka, based on QProblem.cpp by Hans Joachim Ferreau,
 *Andreas Potschka, Christian Kirches \version 3.2 \date 2012-2015
 *
...
returnValue SQProblemSchur::setupAuxiliaryQP(SymmetricMatrix *H_new, Matrix *A_new,
  ...
  if (returnvalue == SUCCESSFUL_RETURN && neig == getNAC()) {
    ...
    if (options.printLevel == PL_HIGH)
      MyPrintf(
          "In hotstart for new matrices, old working set is linearly independent and has correct "
          "inertia.\n");

    status = QPS_AUXILIARYQPSOLVED;
    return SUCCESSFUL_RETURN;
  } else if (returnvalue == SUCCESSFUL_RETURN && neig > getNAC()) {
    /* b) KKT matrix has too many negative eigenvalues. Try to correct the inertia by adding bounds
     * (reduce nullspace dimension). */
    if (options.printLevel == PL_HIGH)
      MyPrintf(
          "WARNING: In hotstart for new matrices, reduced Hessian for initial working set has %i "
...
returnValue SQProblemSchur::setupAuxiliaryWorkingSet(const Bounds *const auxiliaryBounds,
  ...
  for (i = 0; i < nC; ++i) {
    if ((constraints.getStatus(i) == ST_INACTIVE) &&
      ...
      if (constraints.moveInactiveToActive(i, auxiliaryConstraints->getStatus(i)) !=
          SUCCESSFUL_RETURN)
        return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
  }
  ...
  /* III.2.) Check if inertia is correct. If so, we now have a linearly independent working set with
   * a pos def reduced Hessian */
  int_t neig = sparseSolver->getNegativeEigenvalues();
  if (neig == getNAC()) {
  ...
  if (options.printLevel == PL_HIGH)
    MyPrintf(
        "WARNING: In setupAuxiliaryWorkingSet: Initial working set reduced Hessian has %i negative "
...
returnValue SQProblemSchur::addConstraint(int_t number, SubjectToStatus C_status,
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      int_t numNonzerosA;
      A->getSparseSubmatrix(1, &number, icolsLength, icolsNumber, 0, 0, numNonzerosA, irn, jcn,
                            vals);
      delete[] irn;
      ...
      returnValue returnvalue =
          addToSchurComplement(number, SUT_ConAdded, numNonzerosM, MNpos, MNvals, numNonzerosN,
...
returnValue SQProblemSchur::addConstraint_ensureLI(int_t number, SubjectToStatus C_status) {
  ...
  /* 1) Coefficients of linear combination, have already been computed, but we need to correct the
   * sign.  */
  int_t i, ii;
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound == BT_TRUE) {
      ...
      getGlobalMessageHandler()->throwInfo(RET_REMOVE_FROM_ACTIVESET, messageString, __FUNCTION__,
...
farewell:
  ...
  getGlobalMessageHandler()->throwInfo(RET_LI_RESOLVED, 0, __FUNCTION__, __FILE__, __LINE__,
                                       VS_VISIBLE);
    ...
    if (nS < 0 || nS == nSmax) {
      ...
      for (int_t i = 0; i < nFRStart; i++)
        if (FR_idxStart[i] == number) {
          real_t one = 1.0;
          sparse_int_t pos = i;
          if (addToSchurComplement(number, SUT_VarFixed, 1, &pos, &one, 0, 0, 0, 0.0) !=
              SUCCESSFUL_RETURN)
            return THROWERROR(RET_ADDBOUND_FAILED);
          found = true;
          break;
 ...
 */
returnValue SQProblemSchur::removeConstraint(int_t number, BooleanType updateCholesky,
  ...
  /* Now we have a new Schur complement (might be smaller, larger, or empty). Update QR
   * factorization. */
  ...
  if ((options.enableFlippingBounds == BT_TRUE) && (allowFlipping == BT_TRUE) &&
    ...
    if (sModType == 1) { /* Case 1: We added a row and column to S. */
      ...
      if (oldDet * newDet > 0) {
        ...
        switch (oldStatus) {
          ...
          case ST_UPPER:
            ...
            break;
          default: return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
        }
      ...
      /* Check if a direction of negative curvature showed up, i.e. determinants have DIFFERENT
       * signs */
      newDet = calcDetSchur(idxDeleted);
 ...
 */
returnValue SQProblemSchur::removeBound(int_t number, BooleanType updateCholesky,
                                        BooleanType allowFlipping, BooleanType ensureNZC) {
  returnValue returnvalue = SUCCESSFUL_RETURN;
  int_t addIdx;
  BooleanType addBoundNotConstraint;
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      switch (hessianType) {
        case HST_ZERO: N_diag = regularisation; break;

        case HST_IDENTITY: N_diag = 1.0 + regularisation; break;
      ...
      returnvalue = addToSchurComplement(number, SUT_VarFreed, numNonzerosM, Mpos, Mvals,
 ...
 *	s e t u p T Q f a c t o r i s a t i o n
 */
returnValue SQProblemSchur::backsolveT(const real_t *const b, BooleanType transposed,
 ...
 *	b a c k s o l v e R
 */
returnValue SQProblemSchur::backsolveR(const real_t *const b, BooleanType transposed,
                                       real_t *const a) const {
  return THROWERROR(RET_UNKNOWN_BUG);
}
 ...
 */
returnValue SQProblemSchur::backsolveR(const real_t *const b, BooleanType transposed,
                                       BooleanType removingBound, real_t *const a) const {
  return THROWERROR(RET_UNKNOWN_BUG);
}
...
real_t SQProblemSchur::calcDetSchur(int_t idxDel) {
  ...
  /* Case 2: row and column idxDel have been deleted from S */
    ...
    for (i = idxDel; i < nS; i++) {
      computeGivens(tempR[i + i * dim], tempR[(i + 1) + i * dim], tempR[i + i * dim],
                    tempR[(i + 1) + i * dim], c, s);
      nu = s / (1.0 + c);
      /// \todo I think we do not need to transform all columns of R, i+3 or so should be sufficient
      for (j = i + 1; j < nS; j++)
        applyGivens(c, s, nu, tempR[i + j * dim], tempR[(i + 1) + j * dim], tempR[i + j * dim],
                    tempR[(i + 1) + j * dim]);
    ...
    /* Delete row: nS Givens rotations to transform last column (and row!) of (old) Q**T to
    ...
    for (i = nS; i > 0; i--) {
      ...
      applyGivens(c, s, nu, tempR[nS + (i - 1) * dim], tempR[(i - 1) + (i - 1) * dim],
                  tempR[nS + (i - 1) * dim], tempR[(i - 1) + (i - 1) * dim]);
    }
...
returnValue SQProblemSchur::updateSchurQR(int_t idxDel) {
  ...
  if (idxDel < 0) {
    ...
    for (i = 0; i < nS - 1; i++) {
      computeGivens(R_[i + i * nSmax], R_[(nS - 1) + i * nSmax], R_[i + i * nSmax],
                    R_[(nS - 1) + i * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = i + 1; j < nS; j++)
        applyGivens(c, s, nu, R_[i + j * nSmax], R_[(nS - 1) + j * nSmax], R_[i + j * nSmax],
                    R_[(nS - 1) + j * nSmax]);

      /* Simultaneously transform Q**T */
      for (j = 0; j < nS; j++)
        applyGivens(c, s, nu, Q_[j + i * nSmax], Q_[j + (nS - 1) * nSmax], Q_[j + i * nSmax],
                    Q_[j + (nS - 1) * nSmax]);
    }
  }
  /* Case 2: row and column idxDel have been deleted from S */
    ...
    for (i = idxDel; i < nS; i++) {
      computeGivens(R_[i + i * nSmax], R_[(i + 1) + i * nSmax], R_[i + i * nSmax],
                    R_[(i + 1) + i * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = i + 1; j < nS; j++)
        applyGivens(c, s, nu, R_[i + j * nSmax], R_[(i + 1) + j * nSmax], R_[i + j * nSmax],
                    R_[(i + 1) + j * nSmax]);

      /* Simultaneously transform (old) Q**T (coldim is one larger)*/
      for (j = 0; j < nS + 1; j++)
        applyGivens(c, s, nu, Q_[j + i * nSmax], Q_[j + (i + 1) * nSmax], Q_[j + i * nSmax],
                    Q_[j + (i + 1) * nSmax]);
    }
     ...
     * getAbs((nS+1)-th unity vector) */
    for (i = nS; i > 0; i--) {
      computeGivens(Q_[nS + nS * nSmax], Q_[nS + (i - 1) * nSmax], Q_[nS + nS * nSmax],
                    Q_[nS + (i - 1) * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = 0; j < nS; j++)
        applyGivens(c, s, nu, Q_[j + nS * nSmax], Q_[j + (i - 1) * nSmax], Q_[j + nS * nSmax],
                    Q_[j + (i - 1) * nSmax]);

      /* Simultaneously transform R (coldim is already one less than Q) */
      for (j = i - 1; j < nS; j++)
        applyGivens(c, s, nu, R_[nS + j * nSmax], R_[(i - 1) + j * nSmax], R_[nS + j * nSmax],
                    R_[(i - 1) + j * nSmax]);
    }
 ...
 */
returnValue SQProblemSchur::backsolveSchurQR(int_t dimS, const real_t *const rhs, int_t dimRhs,
                                             real_t *const sol) {
  if (dimS < 1 || dimRhs < 1)
    return SUCCESSFUL_RETURN;
...
returnValue SQProblemSchur::stepCalcRhs(int_t nFR, int_t nFX, int_t nAC, int_t *FR_idx,
                                        int_t *FX_idx, int_t *AC_idx, real_t &rhs_max,
...
returnValue SQProblemSchur::stepCalcReorder(int_t nFR, int_t nAC, int_t *FR_idx, int_t *AC_idx,
                                            int_t nFRStart, int_t nACStart, int_t *FR_idxStart,
                                            int_t *AC_idxStart, int_t *FR_iSort,
                                            int_t *FR_iSortStart, int_t *AC_iSort,
                                            int_t *AC_iSortStart, real_t *rhs) {
  int_t i, ii;
...
returnValue SQProblemSchur::stepCalcBacksolveSchur(int_t nFR, int_t nFX, int_t nAC, int_t *FR_idx,
                                                   int_t *FX_idx, int_t *AC_idx, int_t dim,
                                                   real_t *rhs, real_t *sol) {
  returnValue retval;
  int_t i, ii;
  ...
  for (ii = 0; ii < nS; ii++) {
    ...
    switch (
        schurUpdate[ii])  // TODO: All the loops below could be done faster by binary search or so
    {
      case SUT_VarFixed: q[ii] = 0.0; break;
      ...
      case SUT_ConRemoved: q[ii] = 0.0; break;
...
returnValue SQProblemSchur::stepCalcReorder2(int_t nFR, int_t nAC, int_t *FR_idx, int_t *AC_idx,
                                             int_t nFRStart, int_t nACStart, int_t *FR_idxStart,
                                             int_t *AC_idxStart, int_t *FR_iSort,
                                             int_t *FR_iSortStart, int_t *AC_iSort,
                                             int_t *AC_iSortStart, real_t *sol,
                                             real_t *const delta_xFR, real_t *const delta_yAC) {
  int_t i, ii;
  i = 0;
  ii = 0;
...
returnValue SQProblemSchur::stepCalcResid(int_t nFR, int_t nFX, int_t nAC, int_t *FR_idx,
                                          int_t *FX_idx, int_t *AC_idx, BooleanType Delta_bC_isZero,
...
returnValue SQProblemSchur::stepCalcDeltayFx(int_t nFR, int_t nFX, int_t nAC, int_t *FX_idx,
                                             const real_t *const delta_g, real_t *const delta_xFX,
                                             real_t *const delta_xFR, real_t *const delta_yAC,
                                             real_t *const delta_yFX) {
  int_t i;
  for (i = 0; i < nFX; ++i)
  ...
  A->transTimes(constraints.getActive(), bounds.getFixed(), 1, -1.0, delta_yAC, nAC, 1.0, delta_yFX,
                nFX);
...
returnValue SQProblemSchur::determineStepDirection(
  ...
  returnValue retval =
      determineStepDirection2(delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, Delta_bC_isZero,
...
returnValue SQProblemSchur::determineStepDirection2(
  ...
  /* The linear system to be solved here is this:
     ...
     delta_ubA with corresponding sign \  0     I      0     0 / \ -delta_yFX /   \  delta_bX  / <--
     fixed entries of delta_lb and delta_ub with corresponding sign
  ...
  */
  ...
  if (nFR + nAC > 0) {
    real_t rhs_max = 0.0;
    retval = stepCalcRhs(nFR, nFX, nAC, FR_idx, FX_idx, AC_idx, rhs_max, delta_g, delta_lbA,
    ...
    for (r = 0; r <= options.numRefinementSteps; ++r) {
      retval = stepCalcReorder(nFR, nAC, FR_idx, AC_idx, nFRStart, nACStart, FR_idxStart,
                               AC_idxStart, FR_iSort, FR_iSortStart, AC_iSort, AC_iSortStart, rhs);
      if (retval != SUCCESSFUL_RETURN)
        return retval;
      ...
      retval = stepCalcReorder2(nFR, nAC, FR_idx, AC_idx, nFRStart, nACStart, FR_idxStart,
                                AC_idxStart, FR_iSort, FR_iSortStart, AC_iSort, AC_iSortStart, sol,
                                delta_xFR, delta_yAC);
      if (retval != SUCCESSFUL_RETURN)
        return retval;
      ...
      {
        real_t rnrm;
        retval = stepCalcResid(nFR, nFX, nAC, FR_idx, FX_idx, AC_idx, Delta_bC_isZero, delta_xFX,
                               delta_xFR, delta_yAC, delta_g, delta_lbA, delta_ubA, rnrm);
        if (retval != SUCCESSFUL_RETURN)
          return retval;
        ...
        if (options.printLevel == PL_HIGH)
          MyPrintf("In iterative refinement (iter %d) rnrm = %e and epsIterRef*rhs_max = %e.\n", r,
                   rnrm, options.epsIterRef * rhs_max);
  ...
  if (nFX > 0) {
    retval = stepCalcDeltayFx(nFR, nFX, nAC, FX_idx, delta_g, delta_xFX, delta_xFR, delta_yAC,
                              delta_yFX);
    if (retval != SUCCESSFUL_RETURN)
      return retval;
  }
...
returnValue SQProblemSchur::resetSchurComplement(BooleanType allowInertiaCorrection) {
  ...
  int_t numNonzeros;
  switch (hessianType) {
    case HST_ZERO: numNonzeros = 0; break;

    case HST_IDENTITY: numNonzeros = nFR; break;

    default:
      H->getSparseSubmatrix(bounds.getFree(), bounds.getFree(), 1, 1, numNonzeros, 0, 0, 0,
                            BT_TRUE);
      break;
  }
  ...
  A->getSparseSubmatrix(constraints.getActive(), bounds.getFree(), nFR + 1, 1, numNonzerosA, 0, 0,
                        0, BT_FALSE);
  ...
  switch (hessianType) {
    ...
    default:
      H->getSparseSubmatrix(bounds.getFree(), bounds.getFree(), 1, 1, numNonzeros, irn, jcn, avals,
                            BT_TRUE);
      break;
  }
  ...
  A->getSparseSubmatrix(constraints.getActive(), bounds.getFree(), nFR + 1, 1, numNonzerosA,
                        irn + numNonzeros, jcn + numNonzeros, avals + numNonzeros, BT_FALSE);
  numNonzeros += numNonzerosA;
  ...
  if (retval == SUCCESSFUL_RETURN && allowInertiaCorrection) {
    ...
    if (neig > getNAC()) {
      if (options.printLevel == PL_HIGH)
        MyPrintf(
            "WARNING: After new factorization, reduced Hessian has %i negative eigenvalues, should "
            "be %i.\n",
            neig, getNAC());
...
returnValue SQProblemSchur::computeMTimes(real_t alpha, const real_t *const x_, real_t beta,
                                          real_t *const y_) {
...
returnValue SQProblemSchur::computeMTransTimes(real_t alpha, const real_t *const x_, real_t beta,
                                               real_t *const y_) {
...
returnValue SQProblemSchur::addToSchurComplement(int_t number, SchurUpdateType update,
                                                 int_t numNonzerosM, const sparse_int_t *Mpos,
                                                 const real_t *const Mvals, int_t numNonzerosN,
...
returnValue SQProblemSchur::deleteFromSchurComplement(int_t idx, BooleanType allowUndo) {
  if (options.printLevel == PL_HIGH)
    MyPrintf("deleting entry %d with idx = %d and type %d from Schur complement.", idx,
             schurUpdateIndex[idx], schurUpdate[idx]);
...
returnValue SQProblemSchur::repairSingularWorkingSet() {
  ...
  for (k = defect - 1; k > -1; k--) {
    ...
    if (zeroPivots[k] < nFR) {
      ...
      /* Here we do not need to update the Schur complement because KKT matrix is factorized
       * afterwards */
      if (bounds.moveFreeToFixed(number, B_status) != SUCCESSFUL_RETURN)
  ...
  if (options.printLevel == PL_HIGH)
    MyPrintf(
        "WARNING: KKT matrix singular! Removed %i constraints and added %i bounds before "
        "refactorization.\n",
