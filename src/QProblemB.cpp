 ...
 *	i n i t
 */
returnValue QProblemB::init(SymmetricMatrix *_H, const real_t *const _g, const real_t *const _lb,
                            const real_t *const _ub, int_t &nWSR, real_t *const cputime,
                            const real_t *const xOpt, const real_t *const yOpt,
                            const Bounds *const guessedBounds, const real_t *const _R) {
  int_t i;
  int_t nV = getNV();
...
returnValue QProblemB::init(const char *const H_file, const char *const g_file,
                            const char *const lb_file, const char *const ub_file, int_t &nWSR,
 ...
 *	h o t s t a r t
 */
returnValue QProblemB::hotstart(const real_t *const g_new, const real_t *const lb_new,
                                const real_t *const ub_new, int_t &nWSR, real_t *const cputime,
  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    returnvalue = solveRegularisedQP(g_new, lb_new, ub_new, nWSR, cputime, 0, isFirstCall);
  } else {
    ...
    for (;;) {
      ...
      /* Automatically call standard solveQP if regularisation is not active. */
      returnvalue = solveRegularisedQP(g_new, lb_new_far, ub_new_far, nWSR, &cputime_remaining,
                                       nWSR_performed, isFirstCall);
 ...
 *	h o t s t a r t
 */
returnValue QProblemB::hotstart(const char *const g_file, const char *const lb_file,
                                const char *const ub_file, int_t &nWSR, real_t *const cputime,
                                const Bounds *const guessedBounds) {
  int_t nV = getNV();
  ...
  returnvalue = loadQPvectorsFromFile(g_file, lb_file, ub_file, g_new, lb_new, ub_new);
  if (returnvalue != SUCCESSFUL_RETURN) {
    if (ub_file != 0)
      delete[] ub_new;
    if (lb_file != 0)
...
returnValue QProblemB::determineHessianType() {
  ...
  switch (hessianType) {
    ...
    case HST_IDENTITY: return SUCCESSFUL_RETURN;
  ...
  if ((lb_new != 0) && (ub_new != 0)) {
    for (i = 0; i < nV; ++i) {
      if ((lb_new[i] < -INFTY + options.boundTolerance) &&
          (ub_new[i] > INFTY - options.boundTolerance) && (options.enableFarBounds == BT_FALSE)) {
        bounds.setType(i, ST_UNBOUNDED);
      } else {
        if (options.enableEqualities && lb[i] > ub[i] - options.boundTolerance &&
      ...
      if (nFR > 0) {
        ...
        if (info > 0) {
          if (R[0] < 0.0) {
             ...
             * diagonal element. */
            options.epsRegularisation = getMin(-R[0] + options.epsRegularisation,
 ...
 */
returnValue QProblemB::determineDataShift(const real_t *const g_new, const real_t *const lb_new,
 ...
 */
returnValue QProblemB::loadQPvectorsFromFile(const char *const g_file, const char *const lb_file,
                                             const char *const ub_file, real_t *const g_new,
                                             real_t *const lb_new, real_t *const ub_new) const {
  int_t nV = getNV();
 ...
 */
returnValue QProblemB::setInfeasibilityFlag(returnValue returnvalue, BooleanType doThrowError) {
  infeasible = BT_TRUE;
 ...
 */
BooleanType QProblemB::isCPUtimeLimitExceeded(const real_t *const cputime, real_t starttime,
                                              int_t nWSR) const {
...
returnValue QProblemB::performRatioTest(int_t nIdx, const int_t *const idxList,
                                        const SubjectTo *const subjectTo, const real_t *const num,
                                        const real_t *const den, real_t epsNum, real_t epsDen,
 ...
 */
real_t QProblemB::getRelativeHomotopyLength(const real_t *const g_new, const real_t *const lb_new,
 ...
 */
returnValue QProblemB::updateFarBounds(real_t curFarBound, int_t nRamp, const real_t *const lb_new,
 ...
 *	s o l v e Q P
 */
returnValue QProblemB::solveQP(const real_t *const g_new, const real_t *const lb_new,
                               const real_t *const ub_new, int_t &nWSR, real_t *const cputime,
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    ...
    returnvalue =
        determineDataShift(g_new, lb_new, ub_new, delta_g, delta_lb, delta_ub, Delta_bB_isZero);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
      delete[] delta_xFR;
    ...
    returnvalue = determineStepDirection(delta_g, delta_lb, delta_ub, Delta_bB_isZero, delta_xFX,
                                         delta_xFR, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
    ...
    returnvalue = performStep(delta_g, delta_lb, delta_ub, delta_xFX, delta_xFR, delta_yFX, BC_idx,
                              BC_status);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
 ...
 */
returnValue QProblemB::solveRegularisedQP(const real_t *const g_new, const real_t *const lb_new,
  ...
  for (r = 0; r <= options.numRefinementSteps; ++r) {
    ...
    if (nFR > 0) {
      ...
      if ((hessianType != HST_ZERO) && (hessianType != HST_IDENTITY) &&
        ...
        H->times(bounds.getFree(), bounds.getFixed(), 1, -1.0, delta_xFX, nFX, 1.0, delta_xFR_TMP,
                 nFR);
    ...
    if (options.numRefinementSteps > 0) {
      ...
      switch (hessianType) {
        ...
        default:
          H->times(bounds.getFree(), bounds.getFree(), 1, -1.0, delta_xFR, nFR, 1.0, delta_xFR_TMP,
                   nFR);
          H->times(bounds.getFree(), bounds.getFixed(), 1, -1.0, delta_xFX, nFX, 1.0, delta_xFR_TMP,
                   nFR);
 ...
 */
returnValue QProblemB::performStep(const real_t *const delta_g, const real_t *const delta_lb,
  ...
  if (BC_status == ST_UNDEFINED)
    ...
    snprintf(messageString, MAX_STRING_LENGTH, "Stepsize is %.15e! (idx = %d, status = %d)", tau,
             (int)BC_idx, (int)BC_status);
...
returnValue QProblemB::changeActiveSet(int_t BC_idx, SubjectToStatus BC_status) {
  ...
  switch (BC_status) {
    ...
    case ST_UNDEFINED: return RET_OPTIMAL_SOLUTION_FOUND;
    ...
    default:
      ...
      getGlobalMessageHandler()->throwInfo(RET_ADD_TO_ACTIVESET, messageString, __FUNC__, __FILE__,
                                           __LINE__, VS_VISIBLE);
#endif
 ...
 */
returnValue QProblemB::printIteration(int_t iter, int_t BC_idx, SubjectToStatus BC_status,
                                      real_t homotopyLength, BooleanType isFirstCall) {
#ifndef __SUPPRESSANYOUTPUT__
    ...
    case PL_DEBUG_ITER:
      ...
      if ((iter % 10 == 0) && (isFirstCall == BT_TRUE)) {
        snprintf(myPrintfString, MAX_STRING_LENGTH, "\n%5s %4s %4s %9s %9s %9s %9s %9s\n", "iter",
                 "addB", "remB", "hom len", "tau", "stat", "bfeas", "bcmpl");
      }
      myPrintf(myPrintfString);
