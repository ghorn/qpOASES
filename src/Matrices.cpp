...
returnValue Matrix::getSparseSubmatrix(const Indexlist *const irows, const Indexlist *const icols,
  ...
  return getSparseSubmatrix(irows->getLength(), rowsNumbers, icols->getLength(), colsNumbers,
                            rowoffset, coloffset, numNonzeros, irn, jcn, avals,
                            only_lower_triangular);
}

returnValue Matrix::getSparseSubmatrix(const Indexlist *const irows, int_t idx_icol,
                                       int_t rowoffset, int_t coloffset, int_t &numNonzeros,
                                       int_t *irn, int_t *jcn, real_t *avals,
                                       BooleanType only_lower_triangular) const {
  int_t *rowsNumbers;
  ...
  return getSparseSubmatrix(irows->getLength(), rowsNumbers, 1, &idx_icol, rowoffset, coloffset,
                            numNonzeros, irn, jcn, avals, only_lower_triangular);
}

returnValue Matrix::getSparseSubmatrix(int_t idx_row, const Indexlist *const icols, int_t rowoffset,
                                       int_t coloffset, int_t &numNonzeros, int_t *irn, int_t *jcn,
                                       real_t *avals, BooleanType only_lower_triangular) const {
  int_t *colsNumbers;
  icols->getNumberArray(&colsNumbers);

  return getSparseSubmatrix(1, &idx_row, icols->getLength(), colsNumbers, rowoffset, coloffset,
                            numNonzeros, irn, jcn, avals, only_lower_triangular);
}
...
returnValue DenseMatrix::getRow(int_t rNum, const Indexlist *const icols, real_t alpha,
                                real_t *row) const {
  int_t i;
  if (icols != 0) {
...
returnValue DenseMatrix::getCol(int_t cNum, const Indexlist *const irows, real_t alpha,
                                real_t *col) const {
  int_t i;
...
returnValue DenseMatrix::getSparseSubmatrix(
    ...
    const int_t *const icolsNumber, int_t rowoffset, int_t coloffset, int_t &numNonzeros,
    int_t *irn, int_t *jcn, real_t *avals,
...
returnValue DenseMatrix::times(int_t xN, real_t alpha, const real_t *x, int_t xLD, real_t beta,
                               real_t *y, int_t yLD) const {
  ...
  GEMM("TRANS", "NOTRANS", &_nRows, &_xN, &_nCols, &alpha, val, &_leaDim, x, &_xLD, &beta, y,
       &_yLD);
  return SUCCESSFUL_RETURN;
}

returnValue DenseMatrix::transTimes(int_t xN, real_t alpha, const real_t *x, int_t xLD, real_t beta,
                                    real_t *y, int_t yLD) const {
  unsigned long _xN = (unsigned long)xN;
  unsigned long _nRows = (unsigned long)nRows;
  unsigned long _nCols = (unsigned long)nCols;
  ...
  /* Call BLAS. Mind row major format! */
  GEMM("NOTRANS", "NOTRANS", &_nCols, &_xN, &_nRows, &alpha, val, &_leaDim, x, &_xLD, &beta, y,
       &_yLD);
  return SUCCESSFUL_RETURN;
}

returnValue DenseMatrix::times(const Indexlist *const irows, const Indexlist *const icols, int_t xN,
                               real_t alpha, const real_t *x, int_t xLD, real_t beta, real_t *y,
                               int_t yLD, BooleanType yCompr) const {
...
returnValue DenseMatrix::transTimes(const Indexlist *const irows, const Indexlist *const icols,
                                    int_t xN, real_t alpha, const real_t *x, int_t xLD, real_t beta,
                                    real_t *y, int_t yLD) const {
  int_t i, j, k, row, col;
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < irows->length; j++) {
        ...
        for (i = 0; i < icols->length; i++) {
          col = icols->iSort[i];
          y[col + k * yLD] +=
              val[irows->number[row] * leaDim + icols->number[col]] * x[row + k * xLD];
        }
      }
  else if (isEqual(alpha, -1.0) == BT_TRUE)
    for (k = 0; k < xN; k++)
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < irows->length; j++) {
        ...
        for (i = 0; i < icols->length; i++) {
          col = icols->iSort[i];
          y[col + k * yLD] +=
              alpha * val[irows->number[row] * leaDim + icols->number[col]] * x[row + k * xLD];
        }
      }
...
returnValue SymDenseMat::bilinear(const Indexlist *const icols, int_t xN, const real_t *x,
                                  int_t xLD, real_t *y, int_t yLD) const {
  int_t ii, jj, kk, col;
  int_t i, j, k, irA;
...
real_t SparseMatrix::getRowNorm(int_t rNum, int_t type) const {
  ...
  switch (type) {
    ...
    default: THROWERROR(RET_INVALID_ARGUMENTS); return -INFTY;
  }
}

returnValue SparseMatrix::getRow(int_t rNum, const Indexlist *const icols, real_t alpha,
                                 real_t *row) const {
  long i, j, k;
...
returnValue SparseMatrix::getCol(int_t cNum, const Indexlist *const irows, real_t alpha,
                                 real_t *col) const {
  long i, j;
...
returnValue SparseMatrix::times(int_t xN, real_t alpha, const real_t *x, int_t xLD, real_t beta,
                                real_t *y, int_t yLD) const {
  long i, j, k;
...
returnValue SparseMatrix::times(const Indexlist *const irows, const Indexlist *const icols,
                                int_t xN, real_t alpha, const real_t *x, int_t xLD, real_t beta,
                                real_t *y, int_t yLD, BooleanType yCompr) const {
  long i, j, k, l, col;
  real_t xcol;
    ...
    if (xN == 1) {
      ...
      for (l = 0; l < icols->length; l++) {
        ...
        if (isZero(xmax) == BT_FALSE) {
          ...
          for (i = jc[j]; i < jc[j + 1]; i++)
            for (k = 0; k < xN; k++)
              // AW: Maybe it makes more sense to order ytmp by vectors, not vector entries, for
              // better cache peformance?
  ...
  if (yCompr == BT_TRUE) {
    ...
    if (isZero(beta) == BT_TRUE)
      ...
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++)
          y[irows->number[j] + k * yLD] =
              alpha * ytmp[irows->number[j] + k * yfullLength] + beta * y[j + k * yLD];
  }
...
returnValue SparseMatrix::transTimes(const Indexlist *const irows, const Indexlist *const icols,
                                     int_t xN, real_t alpha, const real_t *x, int_t xLD,
                                     real_t beta, real_t *y, int_t yLD) const {
  long i, j, k, l, col;
  real_t yadd;
...
SparseMatrixRow::SparseMatrixRow(int_t nr, int_t nc, int_t ld, const real_t *const v)
    : nRows(nr), nCols(nc), jd(0) {
  int_t i, j, nnz;
...
returnValue SparseMatrixRow::getRow(int_t rNum, const Indexlist *const icols, real_t alpha,
                                    real_t *row) const {
  long i, j;
...
returnValue SparseMatrixRow::getCol(int_t cNum, const Indexlist *const irows, real_t alpha,
                                    real_t *col) const {
  long i, j, k, srt;
...
returnValue SparseMatrixRow::times(int_t xN, real_t alpha, const real_t *x, int_t xLD, real_t beta,
                                   real_t *y, int_t yLD) const {
  long i, j, k;
...
returnValue SymSparseMat::bilinear(const Indexlist *const icols, int_t xN, const real_t *x,
                                   int_t xLD, real_t *y, int_t yLD) const {
    ...
    while (idx < jc[col + 1] && k < icols->length) {
      ...
      if (ir[idx] == row) {
        ...
        if (row == col) /* diagonal element */
          ...
          for (i = 0; i < xN; i++)
            for (j = i; j < xN; j++)
              y[i * yLD + j] += val[idx] * (x[i * xLD + col] * x[j * xLD + row] +
