 ...
 */
returnValue readOqpDimensions(const char *path, int_t &nQP, int_t &nV, int_t &nC, int_t &nEC) {
 ...
 */
returnValue readOqpData(const char *path, int_t &nQP, int_t &nV, int_t &nC, int_t &nEC, real_t **H,
                        real_t **g, real_t **A, real_t **lb, real_t **ub, real_t **lbA,
                        real_t **ubA, real_t **xOpt, real_t **yOpt, real_t **objOpt) {
  char filename[MAX_STRING_LENGTH];
...
returnValue solveOqpBenchmark(int_t nQP, int_t nV, int_t nC, int_t nEC, const real_t *const _H,
                              ...
                              const real_t *const ubA, BooleanType isSparse, const Options &options,
                              int_t &nWSR, real_t &maxCPUtime, real_t &maxStationarity,
                              real_t &maxFeasibility, real_t &maxComplementarity) {
  real_t maxNWSR = 0.0;
  real_t avgNWSR = 0.0;
  real_t avgCPUtime = 0.0;

  returnValue returnvalue = solveOqpBenchmark(
      nQP, nV, nC, nEC, _H, g, _A, lb, ub, lbA, ubA, isSparse, BT_TRUE, options, nWSR, maxNWSR,
      avgNWSR, maxCPUtime, avgCPUtime, maxStationarity, maxFeasibility, maxComplementarity);
  nWSR = (int_t)maxNWSR;
...
returnValue solveOqpBenchmark(int_t nQP, int_t nV, int_t nC, int_t nEC, const real_t *const _H,
                              ...
                              int_t maxAllowedNWSR, real_t &maxNWSR, real_t &avgNWSR,
                              real_t &maxCPUtime, real_t &avgCPUtime, real_t &maxStationarity,
                              real_t &maxFeasibility, real_t &maxComplementarity) {
  int_t k;
 ...
 *	s o l v e O q p B e n c h m a r k
 */
returnValue solveOqpBenchmark(int_t nQP, int_t nV, const real_t *const _H, const real_t *const g,
                              const real_t *const lb, const real_t *const ub, BooleanType isSparse,
                              const Options &options, int_t &nWSR, real_t &maxCPUtime,
                              real_t &maxStationarity, real_t &maxFeasibility,
                              real_t &maxComplementarity) {
  real_t maxNWSR = 0.0;
  real_t avgNWSR = 0.0;
  real_t avgCPUtime = 0.0;

  returnValue returnvalue = solveOqpBenchmark(nQP, nV, _H, g, lb, ub, isSparse, BT_TRUE, options,
...
returnValue runOqpBenchmark(const char *path, BooleanType isSparse, const Options &options,
  ...
  returnValue returnvalue =
      runOqpBenchmark(path, isSparse, BT_TRUE, options, nWSR, maxNWSR, avgNWSR, maxCPUtime,
...
returnValue runOqpBenchmark(const char *path, BooleanType isSparse, BooleanType useHotstarts,
                            const Options &options, int_t maxAllowedNWSR, real_t &maxNWSR,
                            real_t &avgNWSR, real_t &maxCPUtime, real_t &avgCPUtime,
                            real_t &maxStationarity, real_t &maxFeasibility,
                            real_t &maxComplementarity) {
  ...
  if (readOqpData(path, nQP, nV, nC, nEC, &H, &g, &A, &lb, &ub, &lbA, &ubA, 0, 0, 0) !=
      SUCCESSFUL_RETURN) {
  ...
  if (nC > 0) {
    returnvalue =
        solveOqpBenchmark(nQP, nV, nC, nEC, H, g, A, lb, ub, lbA, ubA, isSparse, useHotstarts,
    ...
    returnvalue = solveOqpBenchmark(nQP, nV, H, g, lb, ub, isSparse, useHotstarts, options,
                                    maxAllowedNWSR, maxNWSR, avgNWSR, maxCPUtime, avgCPUtime,
                                    maxStationarity, maxFeasibility, maxComplementarity);
