...
void MA27AD(fint *N, fint *NZ, const fint *IRN, const fint *ICN, fint *IW, fint *LIW, fint *IKEEP,
            fint *IW1, fint *NSTEPS, fint *IFLAG, fint *ICNTL, double *CNTL, fint *INFO,
            double *OPS);
void MA27BD(fint *N, fint *NZ, const fint *IRN, const fint *ICN, double *A, fint *LA, fint *IW,
            fint *LIW, fint *IKEEP, fint *NSTEPS, fint *MAXFRT, fint *IW1, fint *ICNTL,
            double *CNTL, fint *INFO);
void MA27CD(fint *N, double *A, fint *LA, fint *IW, fint *LIW, double *W, fint *MAXFRT, double *RHS,
            fint *IW1, fint *NSTEPS, fint *ICNTL, double *CNTL);
}
 ...
 */
returnValue Ma27SparseSolver::setMatrixData(int_t dim_, int_t numNonzeros_, const int_t *const irn,
...
returnValue Ma27SparseSolver::factorize() {
  ...
  MA27AD(&dim, &numNonzeros, irn_ma27, jcn_ma27, iw_ma27, &liw_ma27, ikeep_ma27, iw1_ma27,
         &nsteps_ma27, &iflag_ma27, icntl_ma27, cntl_ma27, info_ma27, &ops_ma27);
  ...
  MA27BD(&dim, &numNonzeros, irn_ma27, jcn_ma27, a_ma27, &la_ma27, iw_ma27, &liw_ma27, ikeep_ma27,
         &nsteps_ma27, &maxfrt_ma27, iw1_ma27, icntl_ma27, cntl_ma27, info_ma27);
  ...
  } else if (iflag == -5) {  // DJ: I think this is more severe. Can this actually happen?
    rank = -1;
    return RET_KKT_MATRIX_SINGULAR;
  } else if (iflag != 0) {
 ...
 *	s o l v e
 */
returnValue Ma27SparseSolver::solve(int_t dim_, const real_t *const rhs, real_t *const sol) {
  /* consistency check */
  if (dim_ != dim)
  ...
  for (int_t i = 0; i < dim; ++i)
    sol[i] = rhs[i];
  MA27CD(&dim, a_ma27, &la_ma27, iw_ma27, &liw_ma27, w_ma27, &maxfrt_ma27, sol, iw1_ma27,
         &nsteps_ma27, icntl_ma27, cntl_ma27);
 ...
 */
extern void MA57ID(double *cntl, fint *icntl);

/*
 *  MA57AD -- Symbolic Factorization.
 */
extern void MA57AD(fint *n, /* Order of matrix. */
 ...
 */
extern void MA57BD(fint *n, /* Order of matrix. */
                   fint *ne, /* Number of entries. */
 ...
 * MA57CD -- Solution.
 */
extern void MA57CD(fint *job, /* Solution job.  Solve for... */
                   /* JOB <= 1:  A */
                   /* JOB == 2:  PLP^t */
                   /* JOB == 3:  PDP^t */
                   /* JOB >= 4:  PL^t P^t */
...
Ma57SparseSolver::Ma57SparseSolver() : SparseSolver() {
  ...
  // cntl_ma57[1] = 5.0e-16;	// Pivots smaller than this are treated as zero and are placed at
  // the end of the factorization (default: 1e-20) cntl_ma57[0] = 0.5;		// Set pivot
  // tolerance: Higher values = more stable but slower/less sparse (default: 0.01, max 0.5)
}
 ...
 *	s e t M a t r i x D a t a
 */
returnValue Ma57SparseSolver::setMatrixData(int_t dim_, int_t numNonzeros_, const int_t *const irn,
                                            const int_t *const jcn, const real_t *const avals) {
  reset();
  dim = dim_;
  numNonzeros = numNonzeros_;
...
returnValue Ma57SparseSolver::factorize() {
  ...
  MA57AD(&dim, &numNonzeros, irn_ma57, jcn_ma57, &lkeep_ma57, keep_ma57, iwork_ma57, icntl_ma57,
         info_ma57, rinfo_ma57);
  ...
  MA57BD(&dim, &numNonzeros, a_ma57, fact_ma57, &lfact_ma57, ifact_ma57, &lifact_ma57, &lkeep_ma57,
         keep_ma57, iwork_ma57, icntl_ma57, cntl_ma57, info_ma57, rinfo_ma57);
 ...
 *	s o l v e
 */
returnValue Ma57SparseSolver::solve(int_t dim_, const real_t *const rhs, real_t *const sol) {
  /* consistency check */
  if (dim_ != dim)
    return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  for (int_t i = 0; i < dim; ++i)
    sol[i] = rhs[i];
  MA57CD(&job_ma57, &dim, fact_ma57, &lfact_ma57, ifact_ma57, &lifact_ma57, &nrhs_ma57, sol,
         &lrhs_ma57, work_ma57, &lwork_ma57, iwork_ma57, icntl_ma57, info_ma57);
