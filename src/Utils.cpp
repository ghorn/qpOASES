 ...
 *	p r i n t
 */
returnValue print(const real_t *const M, int_t nrow, int_t ncol, const int_t *const ROW_idx,
                  const int_t *const COL_idx, const char *name) {
#ifndef __SUPPRESSANYOUTPUT__
...
returnValue printCopyrightNotice() {
  ...
  myPrintf(
      ...
      "by Hans Joachim Ferreau, Andreas Potschka,\nChristian Kirches et al. All rights "
...
returnValue readFromFile(real_t *data, int_t nrow, int_t ncol, const char *datafilename) {
  ...
  if ((datafile = fopen(datafilename, "r")) == 0) {
    ...
    return getGlobalMessageHandler()->throwError(RET_UNABLE_TO_OPEN_FILE, errstr, __FUNC__,
  ...
  for (i = 0; i < nrow; ++i) {
    for (j = 0; j < ncol; ++j) {
...
#endif /* __USE_SINGLE_PRECISION__ */
      {
        ...
        char errstr[MAX_STRING_LENGTH];
        snprintf(errstr, MAX_STRING_LENGTH, "(%s)", datafilename);
        return getGlobalMessageHandler()->throwError(RET_UNABLE_TO_READ_FILE, errstr, __FUNC__,
                                                     __FILE__, __LINE__, VS_VISIBLE);
      }
 ...
 *	w r i t e I n t o F i l e
 */
returnValue writeIntoFile(const real_t *const data, int_t n, const char *datafilename,
                          BooleanType append) {
 ...
 *	w r i t e I n t o F i l e
 */
returnValue writeIntoFile(const int_t *const integer, int_t n, const char *datafilename,
                          BooleanType append) {
#ifndef __SUPPRESSANYOUTPUT__
 ...
 */
returnValue writeIntoMatFile(FILE *const matFile, const real_t *const data, int_t nRows,
                             int_t nCols, const char *name) {
  ...
  var.numericFormat =
      0000; /* IEEE Little Endian - reserved - double precision (64 bits) - numeric full matrix */
 ...
 *	w r i t e I n t o M a t F i l e
 */
returnValue writeIntoMatFile(FILE *const matFile, const int_t *const data, int_t nRows, int_t nCols,
                             const char *name) {
 ...
 */
returnValue getKktViolation(int_t nV, int_t nC, const real_t *const H, const real_t *const g,
                            const real_t *const A, const real_t *const lb, const real_t *const ub,
                            const real_t *const lbA, const real_t *const ubA, const real_t *const x,
                            const real_t *const y, real_t &stat, real_t &feas, real_t &cmpl,
                            const real_t *const workingSetB, const real_t *const workingSetC,
 ...
 *	g e t K k t V i o l a t i o n
 */
returnValue getKktViolation(int_t nV, const real_t *const H, const real_t *const g,
                            const real_t *const lb, const real_t *const ub, const real_t *const x,
                            const real_t *const y, real_t &stat, real_t &feas, real_t &cmpl,
                            const real_t *const workingSetB, BooleanType hasIdentityHessian) {
  return getKktViolation(nV, 0, H, g, 0, lb, ub, 0, 0, x, y, stat, feas, cmpl, workingSetB, 0,
                         hasIdentityHessian);
}
...
#ifndef __SUPPRESSANYOUTPUT__
  switch (value) {
    case ST_INACTIVE: snprintf(string, 20, "ST_INACTIVE"); break;

    case ST_LOWER: snprintf(string, 20, "ST_LOWER"); break;

    case ST_UPPER: snprintf(string, 20, "ST_UPPER"); break;

    case ST_UNDEFINED: snprintf(string, 20, "ST_UNDEFINED"); break;

    case ST_INFEASIBLE_LOWER: snprintf(string, 20, "ST_INFEASIBLE_LOWER"); break;

    case ST_INFEASIBLE_UPPER: snprintf(string, 20, "ST_INFEASIBLE_UPPER"); break;

    default: snprintf(string, 20, "<invalid value>"); break;
  }
#endif /* __SUPPRESSANYOUTPUT__ */
...
#ifndef __SUPPRESSANYOUTPUT__
  switch (value) {
    case PL_NONE: snprintf(string, 20, "PL_NONE"); break;

    case PL_LOW: snprintf(string, 20, "PL_LOW"); break;

    case PL_MEDIUM: snprintf(string, 20, "PL_MEDIUM"); break;

    case PL_HIGH: snprintf(string, 20, "PL_HIGH"); break;

    case PL_TABULAR: snprintf(string, 20, "PL_TABULAR"); break;

    case PL_DEBUG_ITER: snprintf(string, 20, "PL_DEBUG_ITER"); break;
...
int_t getSimpleStatus(returnValue returnvalue, BooleanType doPrintStatus) {
  ...
  switch (returnvalue) {
    case SUCCESSFUL_RETURN: simpleStatus = 0; break;

    case RET_MAX_NWSR_REACHED: simpleStatus = 1; break;

    case RET_INIT_FAILED_INFEASIBILITY:
    case RET_HOTSTART_STOPPED_INFEASIBILITY: simpleStatus = -2; break;

    case RET_INIT_FAILED_UNBOUNDEDNESS:
    case RET_HOTSTART_STOPPED_UNBOUNDEDNESS: simpleStatus = -3; break;

    default: simpleStatus = -1; break;
  }
 ...
 */
returnValue normaliseConstraints(int_t nV, int_t nC, real_t *A, real_t *lbA, real_t *ubA,
                                 int_t type) {
  int_t ii, jj;
  real_t curNorm;
