...
public:
  ...
  virtual int_t operator()(int_t constrIndex, const real_t *const x,
    ...
    int_t i;
    int_t maxI = (int_t)(((real_t)constrIndex) * ((real_t)nV) / ((real_t)nC)) +
                 diagOffset;
    maxI = getMin(maxI, nV);
...
int main() {
  ...
  readOqpData(path, nQP, nV, nC, nEC, &H, &g, &A, &lb, &ub, &lbA, &ubA, 0, 0,
              0);
  ...
  qp.init(H, &(g[k * nV]), A, &(lb[k * nV]), &(ub[k * nV]), &(lbA[k * nC]),
          &(ubA[k * nC]), nWSR, &cputime);
  ...
  qpCP.init(H, &(g[k * nV]), A, &(lb[k * nV]), &(ub[k * nV]), &(lbA[k * nC]),
            &(ubA[k * nC]), nWSR, &cputime);
