...
int main(int argc, char *argv[]) {
  ...
  if (nproblems == 0) {
    ...
    nproblems =
        scandir("../testing/cpp/data/problems", &namelist, NULL, alphasort);
    if (nproblems <= 0) {
  ...
  for (i = 0; i < nproblems; i++) {
    ...
    if (returnvalue == SUCCESSFUL_RETURN && maxStationarity < TOL &&
        maxFeasibility < TOL && maxComplementarity < TOL) {
      npass++;
