...
int hilbert() {
  ...
  real_t _Av[] = {1.0 / 3.0, 1.0,  0.5, 0.25,      0.25, 0.5,
                  1.0 / 3.0, 0.2,  0.2, 1.0 / 3.0, 0.25, 1.0 / 6.0,
                  1.0 / 6.0, 0.25, 0.2, 1.0 / 7.0};
  /* and its inverse, column major format */
  real_t Bv[] = {240,  16,  -120,  -140,  -2700, -120, 1200, 1680,
                 6480, 240, -2700, -4200, -4200, -140, 1680, 2800};
...
int indexDenseSubmatrix() {
  ...
  for (i = 0; i < m; i++)
    fprintf(stdFile, " %2d", (int)(rNum[i]));
  fprintf(stdFile, "\n");

  cols.getNumberArray(&cNum);
  fprintf(stdFile, "Cols: ");
  for (i = 0; i < n; i++)
    fprintf(stdFile, " %2d", (int)(cNum[i]));
  fprintf(stdFile, "\n");
  ...
  for (j = 0; j < n; j++) {
    for (k = 0; k < K; k++) {
      ...
      if (getAbs(xc[j + k * n]) > errT)
        errT = getAbs(xc[j + k * n]);
    }
  }
...
int spTimes() {
  ...
  errT = 0.0;
  for (i = 0; i < 10; i++)
    if (getAbs(x[i] - ATy[i]) > errT)
      errT = getAbs(x[i] - ATy[i]);
  fprintf(stdFile, "Error in sparse A'*x: %9.2e\n", errT);
...
int symmetry() {
  ...
  real_t ZHZv[] = {0.144, 0.426, 0.708, 0.426, 1.500,
                   2.574, 0.708, 2.574, 4.440};
  ...
  for (i = 0; i < 36; i++)
    Hv[i] = 0.0;
  for (i = 0; i < 6; i++)
    Hv[i * 7] = 1.0 - 0.1 * (real_t)i;
  for (i = 0; i < 5; i++)
    Hv[i * 7 + 1] = Hv[i * 7 + 6] = -0.1 * ((real_t)i + 1.0);
