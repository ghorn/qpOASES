...
int main(int argc, char **argv) {
  ...
  real_t A[6 * 7] = {-0.270792,   -0.140175,   -0.249325,
                     -0.412277,   -0.250952,   -0.298071,
                     1.49078e-19, 0.534738,    0.00704892,
                     0.483388,    -0.193107,   0.302895,
                     -0.2471,     -2.1684e-17, 0,
                     -0.451464,   0.117068,    -0.348254,
                     0.00703103,  0.00620952,  -7.96211e-20,
                     0,           0.0502231,   0.393264,
                     -0.584145,   0.758854,    -0.637889,
                     -0.023751,   0,           0.998738,
                     -0.0197759,  0.766682,    0.632337,
                     0.769921,    0.00352803,  1,
                     4.89658e-12, 0.919213,    0.266407,
                     -0.155855,   0.017872,    -0.999712}; // the jacobian
  real_t g[7];                                             // set to 0
  real_t lb[7]; // joints lower velocity limits
  real_t ub[7]; // joints upper velocity limits
  // Position control - no orientation
  real_t lbA[6] = {0.00442514, -0.122699, -0.0725897,
                   0,          0,         0}; // stylus velocity : ubA = lbA
  real_t ubA[6] = {0.00442514, -0.122699, -0.0725897, 0, 0, 0};
  ;                  // stylus velocity : ubA = lbA
  int_t nWSR = 1000; // specifies the maximum number of working set
                     // recalculation
  real_t cpu_time =
      1.0; // maximum allowed CPU time(s) for the whole initialisation
  real_t xOpt[7];
  ...
  SQProblem joints_velocities(
      7, 6); // create qp problem of 7 variables and 6 constraints with the
             // hessian matrix set with the identity
